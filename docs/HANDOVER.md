...
4. **There is no next CODING task. Everything below is either a user decision or a physical act no
   ...
   **(w) PHASE 30 (Laws of UX applied to the inventory area) — CODE-COMPLETE on
   `phase-30-inventory-uiux` at final tree `4e6fc7f`, UNMERGED and UNPUSHED; no migration.**
   ...
   **Not merged, not pushed.** Merging (`--no-ff` into `main`, then a tree-hash check because the root
   checkout has no `node_modules`), pushing only `main` — which also carries the two docs-only commits
   `a94f47a` (spec) and `b582535` (plan) beyond `origin/main` `c563600` — and the staging redeploy are
   the user's decisions, not pre-authorised. The phase adds no migration, so the redeploy that carries
