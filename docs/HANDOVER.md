...
- **OPEN QUESTION — what "Admin" means (settle before Phase 8's `/admin/users`).** The user has said their "Admin" is short for the **Admin and Purchasing Department**, not a system administrator. Today the app conflates three senses: the workspace labelled `Admin` (`WORKSPACE_META.admin`) whose sidebar is Users & roles / Webhooks / Feature flags — i.e. software settings; the `admin` **role**, a superuser that can enter all four workspaces (`isPermanentAdmin`); and the `/admin/**` URL prefix, which already does double duty (asset categories/types/departments and equipment policies live at `/admin/…` but in the **IT** workspace). The procurement work all sits in the workspace currently called `Purchasing`. Whichever way this settles, the seeded department list (`IT, Finance, Sales, HR, Operations`) is missing an Admin & Purchasing department. Phase 7 is unaffected — `/admin/equipment-policies` is IT-workspace — but Phase 8 builds `/admin/users`, so resolve the naming first.
