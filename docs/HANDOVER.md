...
4. **There is no next CODING task. Everything below is either a user decision or a physical act no
   ...
   **(r) PHASE 25 (IT navigation sweep: start offboarding from the profile, every reference a link,
   ...
   `phase-25-it-navigation-sweep` at final tree `fab0780` (after the final-review fix wave); it is
   UNMERGED and UNPUSHED — merging and pushing are the user's decisions, not pre-authorised — and it
   adds NO migration, so 24 stays and the next staging redeploy is a code-only `-Force` — spec
