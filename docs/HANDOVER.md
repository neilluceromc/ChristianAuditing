...
9. **Running the e2e suite — 227 tests across 18 files, and the split below is MEASURED, not estimated.**
   ...
   **226 / 18 files** at Task 16's close → **227 / 18 files** after the final-review fix wave (D-19 added
   ...
   further count change from D-20 itself). It does **not** fit one foreground run. Split it, each chunk
   with `--global-timeout` so Playwright reaps itself rather than being killed:
