...
9. **Running the e2e suite — 258 tests across 21 files, and the split below is MEASURED, not estimated.**
   ...
   **258 / 21 files, unchanged**, confirmed by `--list` after the final-review fix wave (2026-09-09,
   ...
   `purchasing-ext.spec.ts` case 6), no new case and no new file. It
   does **not** fit one foreground run. Split it, each chunk
   with `--global-timeout` so Playwright reaps itself rather than being killed — **this is the current,
   Phase-17 command list; the five chunk boundaries have not moved since Task 8, only chunk 4's timeout
   (900000, `paging.spec.ts` scale-fixture cases are slower) and chunk 1's (600000, was 540000)**:
...
## 9. Stakeholder requirements from the 2026-09-02 Admin meeting (B and C shipped in Phase 18; D NOT yet planned)
