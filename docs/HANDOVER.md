...
4. **There is no next CODING task. Everything below is either a user decision or a physical act no
   ...
   **(y) PHASE 32 (Laws of UX on the IT screens where the work gets done) — CODE-COMPLETE on
   `phase-32-it-work-uiux` at final tree `e29e879`, UNMERGED and UNPUSHED; no migration.** Spec
   ...
   **Not merged, not pushed.** Merging (`--no-ff`, with a tree-hash check because the root checkout
   has no `node_modules`), pushing only `main` (which also carries the docs-only commits `7b878ed`
   and `4b77f59`), and the code-only staging redeploy are the user's decisions.
