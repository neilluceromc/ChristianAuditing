...
9. **Running the e2e suite — 226 tests across 18 files, and the split below is MEASURED, not estimated.**
   ...
   **226 / 18 files** now (+12 from `registration.spec.ts` and `custody.spec.ts`, one new file each,
   Task 15). It does **not** fit one foreground run. Split it, each chunk with `--global-timeout` so
   Playwright reaps itself rather than being killed:
