...
4. **There is no next CODING task. Everything below is either a user decision or a physical act no
   ...
   **(q) PHASE 24 (IT gaps 2: Replace groups, repairs view paged in SQL, worker retention) IS
   CODE-COMPLETE on `phase-24-it-gaps-2` at final tree `7601cab` (after the final-review fix wave);
   UNMERGED and UNPUSHED, and it adds NO migration — 24 stays, so the next staging redeploy is a
   code-only `-Force` — spec `docs/superpowers/specs/2026-09-17-it-gaps-2-design.md`, plan
   ...
   **What remains: merging to `main`, pushing, and the staging redeploy are the user's decisions, not
   pre-authorised.** There is no migration, so the next `scripts/deploy-staging.ps1 -Force` is a
   code-only redeploy and staging stays at 24. `main` also still carries two unpushed docs commits,
   `0aeb8ce` (spec) and `32e6959` (plan).
