...
4. **There is no next CODING task. Everything below is either a user decision or a physical act no
   ...
   **(x) PHASE 31 (leftovers sweep) — CODE-COMPLETE on `phase-31-leftovers-sweep` at final tree
   `7599d9e`, UNMERGED and UNPUSHED; no migration.** A bounded change with no spec and no plan document
   ...
   **Not merged, not pushed.** Merging (`--no-ff`, with a tree-hash check because the root checkout has
   no `node_modules`), pushing only `main`, and the code-only staging redeploy are the user's decisions.
