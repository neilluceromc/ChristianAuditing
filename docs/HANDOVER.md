...
4. **There is no next CODING task. Everything below is either a user decision or a physical act no
   ...
   **What remains: the merge/push and staging redeploy are the user's decisions (not pre-authorised); D2
   (FIFO costing, expiry, reports) is next; Purchasing's opening stock goes in through `/stock/import`.**
