...
4. **There is no next CODING task. Everything below is either a user decision or a physical act no
   ...
   **What is parked, with a home:** **Phase 26 — reservations that work, and a repair end-date** is the
   next IT candidate (`docs/HANDOVER-PENDING.md` §6, `PICKUP.md` §5): nothing in `src/server` ever
   creates a `Reservation` — only `prisma/seed.ts` does — `HolderControl` exposes only
   `assign`/`return`, and `RELEASED`/`EXPIRED` exist in the enum with nothing transitioning to them, so
   `/reservations`' own copy promises a control that does not exist and a long-expired hold still renders
   as live; and `RETURNED OK` shows `—` in the Down column because `downDays` returns `null` for
   anything not `DEFECTIVE` and nothing records when a repair actually ended. Also parked: the Home age
...
9. **Running the e2e suite — 319 tests across 31 files, and the split below is MEASURED, not estimated.**
   ...
   **Phase 23's Task 10 (2026-09-17) kept Phase 22's seven chunks exactly, adding this phase's two new
   ...
   npx playwright test e2e/custody.spec.ts e2e/axe-sweep.spec.ts e2e/kitchen-sink.spec.ts e2e/suppliers.spec.ts e2e/purchasing-ext.spec.ts --workers=1 --global-timeout=540000
   npx playwright test e2e/stock.spec.ts e2e/stocktake.spec.ts e2e/stock-lots.spec.ts e2e/stock-reports.spec.ts --workers=1 --global-timeout=540000
   npx playwright test e2e/transfers.spec.ts e2e/directory.spec.ts e2e/offboarding-v2.spec.ts e2e/it-gaps.spec.ts e2e/it-nav.spec.ts e2e/oversight.spec.ts e2e/rate-limit.spec.ts e2e/deadlines.spec.ts --workers=1 --global-timeout=540000
   ```
   ...
   `e2e/it-nav.spec.ts` (13 cases) to F — shown in the F line above — so F moves from 42 to **55**,
   every other chunk's count is unchanged, and the battery total is **353**, still inside the cap.
