...
4. **There is no next CODING task. Everything below is either a user decision or a physical act no
   ...
   **Why:** spec §1 decisions 7–11 — per-person exceptions (7), loaner slots (8), loan due date (9), bulk assign (10), signed acknowledgement (11).
   ...
   What remains: **Phase 17** (the scale sweep — pagination everywhere, the employees list in SQL, cursor paging on the merged timelines) is next. Staging runs this code since the 2026-09-08 forced redeploy (`scripts/deploy-staging.ps1 -Force` — the plain run skips when the merge happened on the laptop).

   **(j) PHASE 17 (the scale sweep) IS CODE-COMPLETE on `phase-17-scale-sweep` — spec
   ...
   banner (`D-1`…`D-11` — the final-review fix wave added `D-10`/`D-11`) first, same rule as Phases 11–16.
   ...
   **Battery, measured on `c2b4c9f` (Task 9, 2026-09-08):** `tsc` clean · `lint` clean · **18 migrations** ·
   **1055 unit / 58 files** · **239 e2e / 19 files** across five foreground chunks — **52 (2.8m) · 63 (4.1m)
   · 69 (4.3m) · 49 (3.5m) · 6 (2.9m) = 239**, zero failed, zero did-not-run, no fix needed. See §0 item 9
   for the chunk commands.

   What remains: the merge to `main` and the push, both already authorised by the user for when Phase 17
   is done (progress ledger, 2026-09-08), plus the staging redeploy with `scripts/deploy-staging.ps1 -Force`
   the user separately authorised for after that push. Beyond this phase: the §9 stakeholder subsystems and
   Phase 15b depreciation remain as next-phase candidates.
...
9. **Running the e2e suite — 239 tests across 19 files, and the split below is MEASURED, not estimated.**
   ...
   **226 / 18 files** at Task 16's close → **227 / 18 files** after the final-review fix wave (D-19 added
   ...
   `e2e/paging.spec.ts`, 12 cases, which is the entire 227 → 239 growth; no existing e2e file changed —
   see the plan's `D-1`…`D-11`). It does **not** fit one foreground run. Split it, each chunk
