...
4. **There is no next CODING task. Everything below is either a user decision or a physical act no
   ...
   **(i) PHASE 16 (registration, policy exceptions, loans, bulk assign and the signed form) IS CODE-COMPLETE on `phase-16-registration-custody` (UNMERGED and UNPUSHED) — spec
   ...
   **Why:** spec §1 decisions 7–11 — per-person exceptions (7), loaner slots (8), loan due date (9), bulk assign (10), signed acknowledgement (11).
   ...
   What remains: the merge and push decisions (the user's, as always); **Phase 17** (the scale sweep — pagination everywhere, the employees list in SQL, cursor paging on the merged timelines) is next; staging redeploy applies migration 17.
