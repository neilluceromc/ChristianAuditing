...
4. **There is no next CODING task. Everything below is either a user decision or a physical act no
   ...
   **(u) PHASE 28 (a Back control on every page that has a parent — where the operator came from,
   else the breadcrumb's parent) IS CODE-COMPLETE on `phase-28-back-control` at final tree
   `5a8bbcf` (product commit `ad39271` plus the e2e fix round `5a8bbcf`), UNMERGED and UNPUSHED —
   merging, pushing and the staging redeploy are the user's decisions, not pre-authorised. It is a
