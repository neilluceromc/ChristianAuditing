...
4. **There is no next CODING task. Everything below is either a user decision or a physical act no
   ...
   **§3** `src/lib/approvals-list.ts` gains `CLOSED_VIA`/`parseVia`/`viaWhere`/`CLOSED_VIA_LABEL` and
   ...
   progress, cursor timelines) stay out of scope by the same spec section, and facet queries beside a list
   ...
   **(p) PHASE 23 (Parkinson sweep: quick forms and complete-by dates) IS CODE-COMPLETE on
   `phase-23-parkinson-sweep` at final tree `09bf0a5` — UNMERGED and UNPUSHED (14 commits above `main`
   at `8b2dfd7`; branch and worktree still in place) — spec
...
9. **Running the e2e suite — 319 tests across 31 files, and the split below is MEASURED, not estimated.**
   ...
   **Phase 23's Task 10 (2026-09-17) kept Phase 22's seven chunks exactly, adding this phase's two new
   ...
   and F from 30 to 39, both still far under the ~70-test cap Phase 22's spec §9.3 set.
