...
4. **There is no next CODING task. Everything below is either a user decision or a physical act no
   ...
   **(t) PHASE 27 (IT and quality leftovers sweep: `/audit` hides the approvals, categories and types
   ...
   `phase-27-leftovers-sweep` at final tree `66445ed` (after the final-review fix wave), **UNMERGED and
   UNPUSHED**; it ADDS MIGRATION 26 `20260922010908_case_insensitive_names`, which is PENDING on
   staging until the first `scripts/deploy-staging.ps1 -Force` after a merge —
