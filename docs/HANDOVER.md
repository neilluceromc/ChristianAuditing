...
4. **There is no next CODING task. Everything below is either a user decision or a physical act no
   ...
   **(s) PHASE 26 (holds that work: reserve a spare from the asset record or from an empty profile
   ...
   stamped by the lifecycle preparer) IS CODE-COMPLETE on `phase-26-holds-and-repair-end` at final tree
   `1442289` (after the final-review fix wave); UNMERGED and UNPUSHED — merging, pushing and the
   staging redeploy are the user's decisions, not pre-authorised; it ADDS MIGRATION 25
   `repair_end_and_holds`, so the next staging redeploy after the merge is a `-Force` that applies it —
