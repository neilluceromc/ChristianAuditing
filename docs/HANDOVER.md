...
4. **There is no next CODING task. Everything below is either a user decision or a physical act no
   ...
   **(j) PHASE 17 (the scale sweep) IS CODE-COMPLETE on `phase-17-scale-sweep` — spec
   ...
   banner (`D-1`…`D-9`) first, same rule as Phases 11–16.
...
9. **Running the e2e suite — 239 tests across 19 files, and the split below is MEASURED, not estimated.**
   ...
   **226 / 18 files** at Task 16's close → **227 / 18 files** after the final-review fix wave (D-19 added
   ...
   `e2e/paging.spec.ts`, 12 cases; the rest of the growth is axe assertions and new cases inside existing
   files — see the plan's `D-1`…`D-9`). It does **not** fit one foreground run. Split it, each chunk
