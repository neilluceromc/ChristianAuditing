...
4. **There is no next CODING task. Everything below is either a user decision or a physical act no
   ...
   **`npm run worker:once` now prunes as a side effect**, and four specs shell out to it
   ...
   stays uniform. The three IT items that remain open and unchanged are `/audit` class scoping of
   approval, category and type rows; case-insensitive uniqueness for reference data; and the activity
   page's action facet and import-update sentences.
   ...
   **What is parked, with a home:** **Phase 26 — holds that work, and a repair end-date** was the
   ...
   Phase 24, and the three IT items still open (`/audit` class scoping of approval, category and type
   rows; case-insensitive uniqueness for reference data; the activity page's action facet and its
   import-update sentences). Five findings were recorded rather than fixed: the HOLD holder link
   ...
   **What stays parked, with a home:** the three IT items still open (`/audit` class scoping of
   approval, category and type rows; case-insensitive uniqueness for reference data; the activity page's
   action facet and its import-update sentences); the `role="group"` ARIA shape from Phase 24; the Home
   ...
   **M-9**, `holdFacets` runs its four queries after the paged snapshot and lists every employee with a
   ...
   `P2002` catch; and **`downDays` counts elapsed 24 h periods, not `Asia/Manila` calendar days**, which
...
9. **Running the e2e suite — 319 tests across 31 files, and the split below is MEASURED, not estimated.**
   ...
   **Phase 26 keeps all seven too**, adding its one new file `e2e/holds.spec.ts` (11 cases) to F —
   shown in the F line above — so F moves from 55 to **66**, every other chunk's count is unchanged,
   and the battery total is **364**, still inside the cap.
