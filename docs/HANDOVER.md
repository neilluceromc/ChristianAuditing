...
4. **There is no next CODING task. Everything below is either a user decision or a physical act no
   ...
   **258 / 21 files**, `npm run db:seed` re-run. Report:
   ...
   removal. **Staging is NOT yet redeployed** — migration 19 (`purchasing_suppliers`, additive plus the
   `importedAt` backfill) applies at the next `scripts/deploy-staging.ps1 -Force`. What remains: that
   redeploy; Purchasing's cleaned supplier list goes in through `/purchases/suppliers/import`; item D is next.
