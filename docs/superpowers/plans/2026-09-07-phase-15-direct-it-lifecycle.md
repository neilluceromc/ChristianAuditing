...
>
> **D-9. Post-review — the scanner's "already decided" verdict for IT items (closes the question D-4 left open, chip `task_4e151341`):** the wizard fed `ScanProvider` only `items.filter(i => i.held)`, so a direct decision — which clears the holder in the same transaction — dropped the item from the scan pool at once, and a re-scan of that sticker read "not one of this person's items" instead of "already decided". Fixed by feeding the whole union `items` already carries (held ∪ decided-and-gone): every gone item carries a decision, so it can only yield `already-decided`, never `match`. In the same step the collect banner's "Nothing moves until the approval executes" was false for direct items; it now describes whichever paths the viewer's held items are on, keyed on `isDirectLifecycle` exactly like the cards, and is rendered only for a viewer who can decide. `e2e/scanner.spec.ts` regains the IT case (BR-HS-0510, asserting `assigneeId` is null before the re-scan) and keeps D-4's Purchasing car for the held-and-still-decided half — **214 e2e / 16 files** by `--list`, one over the 213 the Phase 15 battery ran; `scanner` + `offboarding` (23 cases) re-ran green after the change. Lesson: when a write changes which set an item belongs to, every reader that filtered on the old set needs re-checking — the scan pool was one, and the banner copy beside it was another.
