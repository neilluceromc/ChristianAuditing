...
## Amendments made during execution (`D-1`…`D-10`, from the SDD ledger, 2026-09-22)

Pre-flight rulings:

- **D-1** (rulings R1–R4, before Task 1) — R1: the worktree was branched from LOCAL `main` (8c24c3b) with `git worktree add`, so the spec (385b484) and plan (8c24c3b) commits are in the branch's history as in Phases 20–26 (cost if wrong: none — the merge is `--no-ff` into the same `main`). R2: Task 1's end gate accepted exactly three tsc errors (`audit/queries.ts`, `audit/page.tsx`, `audit/export/route.ts` — `string[]` where `HiddenAuditRefs` was now expected), cleared by Task 2 first. R3: P-1's `T2 ∥ T3` ran sequentially, T2 first — both gates run whole-tree tsc and T3 walks a dev server (the Phase 25 lesson). R4: the plan's `parseIntParam("page=0", "page", 1)` expectation was wrong (`0 || 1` is 1 — `||` reads 0 as absent, the behaviour both hand-rolled parsers already had); the brief expects 1.

Task rulings:

- **D-2** (Task 1, 72f644f) — `src/server/prisma-errors.ts` (which already existed with `asActionResult`) gained `isUniqueViolation` and `uniqueTarget` (both `meta.target` shapes) with a test; `src/lib/audit-list.ts` gained `HiddenAuditRefs`/`NO_HIDDEN_REFS` and `buildAuditWhere(state, hidden)` emitting one `NOT: { OR: [...] }` per non-empty list, plus the four entity types the page already rendered (`vendor`, `stock-item`, `stock-category`, `stocktake`); `src/lib/activity-list.ts` (new) with the four feeds, `ACTIVITY_LIST_CONFIG`, `feedWhere` (the finance predicate moved here from `finance/queries.ts`) and `buildActivityWhere`; `src/lib/activity.ts` gained nine sentences (documents, loan due date, secret, acknowledgement, three policy exceptions, supplier set/cleared), `ACTION_LABELS`/`actionLabel` and `FIELD_LABELS`/`fieldLabel` (the `update`/`import-update` sentences print human field names); `parseIntParam` in `paging.ts` used by `parsePage` and `parseTimelineCursor`, the cuid byte-order comment on the timeline tiebreaker; `schema.prisma` gained `@@index([entityType, action])`; migration 26 `20260922010908_case_insensitive_names` (Prisma's index plus seven raw `lower()` unique indexes) applied to `inventory_dev`, 26 up to date; the seed's `CM-0002` issue moved to day −11. Suite 87 files / 1527 tests (+2 / +16); tsc (three R2 errors only), eslint clean. Review: spec ✅, quality approved, 0 findings.
- **D-3** (Task 2, 38c74b7) — `invisibleAuditRefs(role)` (the SEE map; `invisibleAssetIds` kept for the asset list, P-8; class-less approvals never hidden; four empty lists and no query for an all-class role); `listAudit(state, hidden)`; `listActivity(feed, state, hidden)` with the action `groupBy` inside the paged snapshot's count closure (P-9), entity chips on the inventory/employees feeds, the domain pill on finance, options sorted by count then label; `financeActivityWhere` removed (P-7; its one caller bridged until Task 4); `/audit` page and export on refs; the `vendor` → "Supplier" facet-label override (P-10). Proof on `inventory_dev`: IT hides 7 assets / 4 categories / 8 types, admin hides nothing; a filtered feed keeps every facet option. Deviation: the `groupBy` assignment split into two lines for Prisma's generic inference. Review: spec PASS, quality accept, 1 Minor — **M-P27-1** the doc comment in `activity-list.ts` names the removed constant; ruled kept as provenance.
- **D-4** (Task 3, 9c1883e) — case-insensitive pre-checks inside each write's transaction, before the write, naming the existing row (categories/types/departments `That name is already taken by "…"`, policies `That policy name is already taken by "…"`, suppliers `A supplier with this name already exists: "…"`, stock categories by name and by prefix `That prefix is already in use by "…"`); renames exclude the row itself, types are scoped by category; the policy check surfaces through a thrown marker past the module's private `asActionResult`; the five P2002 helper copies replaced by the shared pair (inventory's weaker local `uniqueTarget` deleted; its two approval-race catches keep their messages); supplier documents order by created date then id; the supplier runner drops its two unused setters. Walk on 3100: every refusal seen. **Ruling R5** — the plan's grep gate ("P2002 only in prisma-errors.ts") was wider than the spec: six pre-existing inline P2002 checks (suppliers/bank-actions, admin/webhook-actions, approvals, employees, offboarding, receiving, reservations actions) stay; recorded for a later mechanical swap. Review: spec PASS, quality PASS, 1 Minor — **M-P27-2** `renameRefRow`'s type-scoping `findUniqueOrThrow` can throw P2025 in a tiny window (a pre-existing exposure of the following `update`); parked.
- **D-5** (Task 4, 5e47283) — `ActivityToolbar` (the Action facet, Clear while a selection is on, the live entry count) and `ActivityListPage` (one server body: parse → refs → `listActivity` → toolbar → feed → pagination keeping the filter; filtered-empty "No entries match this filter") behind four five-line pages (P-2). Walk on 3100 as IT/purchasing/finance/viewer: every checkpoint; the finance domain pill and a filtered page 2 verified by source because the fresh seed had no such rows. Review: spec PASS, quality PASS, 0 findings.
- **D-6** (Task 5, 8416635) — `EntityCombobox` opens on click, typing or ArrowDown, never on focus alone; the receive form's Supplier options lead with "No supplier"; `Stat` gained `tone` (`text-accent` + `data-tone`, P-3) and the Purchasing "Stocktakes past close-by" tile turns accent when overdue; the work page's capped note counts rendered rows; `rowOpenProps` in `table.tsx` spread by the inventory (new Enter support), employees and holds tables. The combobox blast radius (P-5) ran green: custody+department-owned+direct-lifecycle+registration 39 passed (3.4m); holds+it-gaps+quick-forms+stock+stock-lots 48 passed (4.5m) after one sanctioned test fix (`e2e/quick-forms.spec.ts:265,273` `.focus()` → `.click()`). Review: spec PASS, quality PASS, 1 Minor — **M-P27-3** the note change is a no-op for today's one capped caller; ruled kept (the note states what it renders).
- **D-7** (Task 6, 2505d7d) — `e2e/activity.spec.ts` (7 cases: the facet narrows/URL/paging/Clear, IT sees no Purchasing rows, a document sentence, an import-update with human field names, Enter on an `/inventory` row, the receive page's comboboxes and "No supplier", the accent tile with an overdue stocktake), admin +3 (case-only category refused, same-row case rename accepted, department refused), stock +1, suppliers +1, approvals-audit +1 (class hygiene), it-gaps cases 1/3 with `try/finally`. Chunks 29 passed (2.3m) and 39 passed (2.9m); `--list` 377 / 36 (13 new cases — the plan's 378 counted the two it-gaps edits as additions). Adaptations (P-4 and ten recorded): the seed has no `lifecycle.assign` audit row so case 1 writes its own; `?q=<exact tag>` redirects to the record; no seeded approval hangs off a Purchasing asset; `p[role="alert"]` (Next's route announcer answers `getByRole("alert")`). Review: spec PASS, quality PASS, 0 findings; two out-of-scope facts confirmed — `entityLabels` had no `asset-category`/`asset-type` branch, and jsonb reorders diff keys.
- **D-8** (final whole-branch review, opus, 8c24c3b..2505d7d) — Approved with fix wave, 0 Critical / 2 Important / 6 Minor. **I-1** the approvals worker writes five `<lifecycle.*> executed` asset actions that reached the feeds with no label and no sentence. **I-2** `entityLabels` printed truncated cuids for the two entity types this phase class-scopes. Minors: M-1 field lists in jsonb key order (unit test and e2e pinned opposite orders), M-2 `/audit`'s entity pill still renders raw `vendor`, M-3 a stray `?q=` sticks to the feeds' hrefs, M-4 it-gaps case 3's fixtures outside its `try`, M-5 the `""` clear option's id/key, M-6 the `document.signed` single-key contract undocumented. Positions: M-P27-1/2/3 stay; the six inline P2002 checks stay (R5); `data-tone` acceptable. **Ruling R6** set the wave: I-1, I-2, M-1 (sort on the label), M-3, M-4, M-5, M-6; record-only M-2 (needs its own e2e pass — HANDOVER (t)).
- **D-9** (final-review fix wave, 66445ed; ruling R6) — `entityLabels` resolves `asset-category` (the name, linking to `/admin/asset-categories`) and `asset-type` (`Category · Type`, linking to `/admin/asset-types`) — the file's majority convention for admin rows rather than `department`'s unlinked label; the approvals worker's five `<lifecycle.*> executed` asset actions run through the lifecycle sentence bodies with the suffix " (approved request)" and carry facet labels ("Assigned (approved request)" …), and `lifecycle.transfer` gained its own sentence and label ("Asset transferred"); `fieldList` sorts on the label, so an `update`/`import-update` sentence lists fields in one stable order whatever jsonb stored (two pre-existing unit assertions re-pinned: "assignee, status", "cost, model"); the feeds' pagination and facet hrefs drop a stray `?q=`; `e2e/it-gaps.spec.ts` case 3 creates its fixtures inside its `try`; the `""` clear option is keyed and identified as `none` in `EntityCombobox` (with the sentinel commented on the receive form); the `document.signed` single-key contract is commented at the writer and the reader. Forced test adaptations: the hygiene e2e locates the category row by its name (the cuid workaround gone); activity case 1 picks the facet option by its exact label span (the new "Assigned (approved request)" label made `getByLabel("Assigned")` ambiguous). Verification: tsc, eslint clean; vitest 87 / 1528; Playwright activity + approvals-audit + it-gaps `29 passed (2.3m)`; seed last. Re-review (sonnet): CLEAN — every finding addressed, nothing new; one note that the asset history tab still prints the raw `lifecycle.assign executed` string by design (a raw-trail path, not a feed). Record-only, per the final review: **M-2** `/audit`'s entity pill renders the raw `vendor` while the facet and chip say "Supplier" (a one-line `humanize` on the pill, but the pill is CSS-uppercased and an existing e2e asserts `ASSET` on it — needs its own e2e pass); the six inline P2002 checks outside the five modules (R5); `Stat`'s `data-tone` (state attributes already ship in this codebase). Spec amendments this entry drives: §3.5 (the six `executed`/transfer labels, the " (approved request)" suffix, the sorted field list), §4.1 (`entityLabels`' two branches), §5.5 (the `none` sentinel).
- **D-10** — Measured at close (Task 7, final tree `66445ed`, 2026-09-22). `npx tsc --noEmit` clean; `npx eslint .` clean; `npx vitest run` **87 files / 1528 tests**, all passed; `npx prisma migrate status` **26 migrations found in prisma/migrations**, “Database schema is up to date!” — migration 26 `20260922010908_case_insensitive_names` is this phase's, additive (one ordinary index on `AuditEntry (entityType, action)` plus seven `lower()` unique indexes) and it FAILS LOUDLY on a database already holding two names that differ only by case; staging was checked clean on 2026-09-22 and the duplicate check to run before any redeploy is in HANDOVER (t), PICKUP's Database row and HANDOVER-PENDING §1. `npx playwright test --list` **Total: 377 tests in 36 files**. The seven foreground chunks, `E2E_PORT=3100 --workers=1 --global-timeout=540000`, one at a time with port 3100 proved free before each and after the last, `e2e/activity.spec.ts` in **E2** (P-6): **A 54 passed (3.5m) · B 67 passed (3.9m) · C 64 passed (4.2m) · D 50 passed (3.7m) · E1 37 passed (5.8m) · E2 39 passed (3.6m) · F 66 passed (6.6m) = 377**, zero failed, zero flaky, zero skipped, zero did-not-run — every chunk green on its first pass, no re-run and no test-only fix commit anywhere in the battery. Every count matched the expectation exactly (364 → 377: C 63→64, D 47→50, E1 36→37, E2 31→39 for activity.spec's 7 plus stock's 1, F unchanged at 66) — the first phase to grow E2 rather than F. `npm run db:seed` ran last, clean; port 3100 carried no listening process before the first chunk, between any two chunks, or after the seed. **The branch is UNMERGED and UNPUSHED and migration 26 is pending on staging; merging, pushing and the redeploy are the user's decisions.** Documents written in one commit: `docs/HANDOVER.md` (line 3's leading parenthetical, the new **(t)** block after (s), §0 item 9's chunk E2 line and its narrative, §6a rule 22's `actionDot` note, §7's audit-reader gotcha, §8's Phase 9 Tasks 9–12 and Phase 7 Task 14 entries, blocks (q)/(r)/(s)' three IT items, and both raw-SQL-only invariant lists — now twelve), `docs/PICKUP.md` (Branch, Database, a new Battery row, Last two phases with Phase 25's rolling off, §4's new item 1 with the rest renumbered, §5), `docs/HANDOVER-PENDING.md` (the header, §1 rewritten, §5.2, §5.3, §6), this plan's D-block, and the spec's Status line with its `*Amended (D-n):*` notes.
