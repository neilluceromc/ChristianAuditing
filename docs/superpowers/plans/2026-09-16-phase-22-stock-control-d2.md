...
## Amendments made during execution (`D-1`…`D-20`, from the SDD ledger, 2026-09-16 — the final-review fix wave added `D-16`…`D-19`, the final battery `D-20`)
...
- **D-16** (final review, Important 1) — `computeExpiryLots` selected every dated lot with units remaining, archived items included, while the Home tile (`stockHomeSignals`) and the on-hand report both exclude archived items; archiving an item with an expiring lot made the tile read 0 while the report it links to still listed the lot with a live Write off button. The expiry report and its export now exclude archived items. Commit `2eeabf7`.
- **D-17** (final review, Important 2) — the importer's opening-stock inflow (added in D-9) never revalidated the three report pages — the same omission R6 fixed in the other two writers. The six revalidation calls now live in one shared helper beside `ledger.ts` and every stock writer (movement actions, stocktake posting, the import apply) calls it. Commit `2eeabf7`.
- **D-18** (final review, Minor 3 and Minor 4, adjacent) — the expired-shortfall sentence ("write off the expired lot first") fired under the `"first"` policy too, where the expired units had already been consumed and the advice was a no-op; it is now gated on `policy === "skip"`. `expiryLabel` rendered "1 days"; it now says "day" for one. Commit `2eeabf7`.
- **D-19** (final review, parked Minors, rulings) — 5: the expiry table's Days cell uses `--danger-bg` (a button-fill token) as text while the Lots card uses `--st-fault-text` for the same state (park; a token pass belongs to a design revision); 6: `receiptSchema`'s expiry-vs-lot-date refinement is skipped when Lot date is blank (the server defaults lot date to today; park); 7: the attach-document dialog does nothing, silently, when no file is chosen (park; a one-line "Choose a file" field error for a later touch); 8: `ExpiryTable` re-derives "expired" by sniffing the label string instead of reading the row's flag (park).
- **D-20** — Measured at close, final tree `a4b57b5` (after the fix wave; product code untouched since): `tsc` clean · `lint` clean · **1404 unit / 79 files** · **23 migrations**, schema up to date · **319 e2e / 31 files** by `--list` — seven foreground chunks with explicit file paths, `E2E_PORT=3100 --workers=1 --global-timeout=540000` — **A 54 passed (3.3m) · B 67 passed (3.9m) · C 63 passed (4.0m) · D 38 passed (2.6m) · E1 36 passed (5.7m) · E2 31 passed (3.1m) · F 30 passed (3.3m) = 319**, zero failed, zero did-not-run. Chunk A's first pass was 53 passed, 1 failed (`e2e/it-core.spec.ts:302`, the documents-upload PNG-rejection case, a `toBeVisible` 5 s timeout on `getByText(/Accepted: PDF, PNG, JPG/)`) and failed identically on an immediate as-is re-run — reproduced, not the cold-compile/headroom class. Diagnosis: `setInputFiles` fires its native `change` event the instant the server-rendered `<input>` exists, before React's delegated `onChange` is wired up on a freshly-navigated page; the file's own `waitForHydration()` helper (already wired to one call site) was never wired to this test's upload call. Fixed test-only: `await waitForHydration(page.getByLabel("Upload document"));` before the first `setInputFiles()`, verified in isolation, then chunk A re-ran 54/54 clean (3.3m). Commit `a4b57b5`.
