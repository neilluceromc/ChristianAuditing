...
## Amendments made during execution (`D-1`…`D-10`, from the SDD ledger, 2026-09-21)

Pre-flight and early rulings:

- **D-1** (ruling R1) — the worktree was branched from LOCAL `main` (611d91f) with `git worktree add`, not the harness's `fresh` base (`origin/main` 6530623), so the spec (2bed93b, 611d91f) and plan (611d91f) commits sit in the branch's history as in Phases 20–25. Cost if wrong: none (the merge is `--no-ff` into the same `main`).
- **D-2** (spec corrections before the plan, 611d91f) — the plan facts corrected four spec details: the profile's Reserve affordance is the empty slot's `⋯` menu item "Reserve a spare…" (the whole empty tile is already the Assign affordance); e2e case 2 uses Nina Robles (EMP-0097, the only seeded employee with an equipment policy — Carlo Dizon has none, so his profile shows no slots); `canAssign` on the asset record stays unchanged (custody case 9 assigns a held asset to its holder); `ReservationRow` gains `created` for the sortable Created column.

Task rulings:

- **D-3** (Task 1, d19f854) — migration 25 `20260921072537_repair_end_and_holds` (the `ALTER TABLE` plus the audit-history backfill; applied to `inventory_dev`, 25 up to date, no drift) and `Asset.repairEndedAt`; `src/lib/holds.ts` (defaults, `holdStatus`, `isHoldExpired`, the hourly gate, the tabs moved out of the server module, `HOLDS_LIST_CONFIG`, `buildHoldWhere`/`buildHoldOrderBy`) with 8 tests; `downDays` closes on `repairEndedAt` (3 tests), the twelve fixture rows widened; hold reason chips; `reservation.placed`/`reservation.released` sentences and dots; `HoldPill`; `stageOf`/`toRow`/Home rows pass the new field. **Ruling R2** — the implementer also edited `e2e/it-gaps.spec.ts` (two type-only lines: its case 6 builds `RepairLike` objects), outside the task's file list, because tsc must be green at the end of every task; accepted and disclosed. Suite 85 files / 1511 tests (+18); tsc, eslint clean. Review: spec ✅, quality approved, 2 Minor — **M-P26-1** parked (the tabs are imported and re-exported in two statements in `reservations/queries.ts`; Task 4 may tidy it while it edits that file), the disclosed deviations blessed.
- **D-4** (Task 2, de23643) — `prepareLifecycle` stamps `repairEndedAt` (cleared on entering DEFECTIVE, set on leaving; both in the audit diff), `LifecycleAsset`/`assetSelect` widened; `reserveAsset` and `releaseHold` (direct admin/IT actions, guard → rate → zod → transactional loads and refusals with the spec's copy, one audit entry each — `reservation.placed` / `reservation.released`, six revalidations); `listReservations(tab, state)` keeps the tab counts inside the snapshot, applies `buildHoldWhere`/`buildHoldOrderBy`, exposes `expiresAt`, reads `resolved` from `resolvedAt` first, and returns Employee/Department facets narrowed by the other facet and the search; `activeHoldFor`; `AssetRow.hold.expiresAt`; the worker's `expireHolds` (cutoff = start of today, Asia/Manila) with `safeExpire` at start and hourly. Proofs against `inventory_dev`: `activeHoldFor(BR-MN-0910)` → Nina Robles +7 d; `listReservations` with `q=nina` → 1 row and her facet count; a temporary two-day-old hold → `npm run worker:once` printed `[worker] expired 1 hold`, the row EXPIRED with `resolvedAt`, then removed. Brief typo corrected: `reason: reasonOptional()` (the call), as every other schema in the codebase. Suite 85 / 1511; tsc, eslint clean. Review: spec ✅, quality approved, 3 Minor parked — **M-P26-2** the check-then-create window under READ COMMITTED (the codebase's existing convention), **M-P26-3** a redundant `.trim()` after `cleanReason`, **M-P26-4** the Employee facet list is not narrowed by the Department facet (dimmed zero counts, the sibling convention).
- **D-5** (Task 3, 34f367c) — `HolderControl` gains `mode: "reserve"` (For combobox with recent picks and focus, Expires prefilled to today + 7 with `min`, `hold.place` chips, toast) and `heldFor` on assign mode (the holder preselected, the held line under the field); `ReleaseHoldButton` (confirm dialog, optional `hold.release` reason, `stopPropagation` opener); the record layout loads `activeHoldFor`, adds `canReserve`/`canReleaseHold` (`canAssign` untouched), renders the Reserve control and the held banner (holder link, `HoldPill` with date, reason, employee number, Release); the Reservations tab shows the pill and Release on the ACTIVE row; the record's Repair card reads "N d out of service" / "down N d, back since <date>" / "clock stopped"; `InventoryTable` gets `today` and the HOLD cell appends the expiry text after the existing pill and holder link. Walk on 3100 as IT/viewer: every Step 5 checkpoint seen, nothing submitted. Deviation: `canManageClass` not imported on the holds tab page (`isDirectLifecycle` composes it). Suite 85 / 1511; tsc, eslint clean. Review: spec ✅, 1 Important — the Assign dialog preselected a holder who might no longer be in the ACTIVE employee options, leaving the combobox blank; **Ruling R3**: fix round 1 (b81dd5e) preselects the holder only when present in the options, the held caption stays unconditional; re-review ADDRESSED. 2 Minor: **M-P26-5** a redundant `!isReserve` conjunct (parked), the `Banner` title rendering as a paragraph (house convention).
- **D-6** (Task 4, 7ed86e9) — the empty slot's `⋯` menu leads with "Reserve a spare…" (dialog: unheld spares same-type first, Expires prefilled with `min`, `hold.place` chips, `reserveAsset`, toast); `HoldingItem` gains `reservationId`/`expiresAt` and reserved rows show `HoldPill` + Release; spares held for someone else are disabled in all three pickers; `/reservations` rebuilt on `parseListState(HOLDS_LIST_CONFIG)` + the `state=` tab — `HoldsToolbar` (search, Employee and Department facets, total), `HoldsTable` (sortable Asset/For/Expires/Created, whole-row click with the selection guard and Enter, inner links stop propagation, `HoldPill` with date on ACTIVE rows, the Closed cell's "expired"/"released" words, Release on ACTIVE rows for admin/IT), the banner's last sentence rewritten, three empty states, pagination keeping tab/search/facets; `ReservationRow.created`. Walk on 3100 as IT/viewer: every Step 3 checkpoint seen; the Expires header reaches `sort=-expiresAt` in one click because ascending is the default; nothing submitted. Suite 85 / 1511; tsc, eslint clean. Review: spec ✅, quality approved, 2 Minor parked — **M-P26-6** the Actions header renders for viewers too (an empty column), **M-P26-7** disabled held-spare radios are skipped by arrow-key traversal while their labels stay visible.
- **D-7** (Task 5, f1979b0) — `e2e/holds.spec.ts`, 11 cases: reserve from the record (default date, hold row, banner and HOLD-cell text, the audit sentence, the spare gone from BR-LT-0201's Replace picker), reserve from Nina Robles' empty phone slot, the held record (no Reserve, preselected holder, refused reassignment naming EMP-0097), the date floor with `min` stripped, release from the record, from the profile and from `/reservations`, fulfilment by assigning the holder, the worker's expiry sweep through `npm run worker:once`, `/reservations` parity (search, Employee facet, the one-click `-expiresAt` flip, row click, the viewer without Release), backfill honesty on BR-MN-0911 and the DEFECTIVE → SPARE → DEFECTIVE stamps. Runs 11 passed (1.3m) twice; neighbours (`offboarding`, `paging`, `custody`, `it-nav`) 45 passed (3.7m); `--list` 364 tests / 35 files (P-7). Accepted deviations: the asset timeline prints action names, not sentences, so cases 1/5 assert the timeline point and the sentence on `/inventory/activity`; case 4 drives the person before the date (a combobox flake otherwise); case 7 leaves its EXECUTED approval, audit rows and RecentPick (P-8, no audit deletes). Review: spec ✅, quality approved, 1 Minor parked — **M-P26-8** case 6 creates two fixtures before its `try`; reviewer's run 11 passed (1.3m).
- **D-8** (final whole-branch review, opus, 611d91f..f1979b0) — Approved with fix wave, 0 Critical / 3 Important / 9 Minor. **I-1** `reserveAsset` had no `P2002` branch although the partial unique index `Reservation_one_active_hold_per_asset` (migration `20260814090100`) enforces one live hold per asset — a concurrent reserve threw past the action instead of returning the pinned conflict copy; spec §0 decision 8's premise ("the codebase has never had a partial unique index") was wrong and is corrected as an *Amended* note. **I-2** the `/reservations` tab badges counted unfiltered while the tab links carried the search and facets. **I-3** `holdStatus` compared Manila calendar days while the sweep compared raw instants against Manila midnight — equal only for UTC-midnight rows, and the seed wrote wall-clock `expiresAt`; `isHoldExpired` had no consumer and `HOLDS_SWEEP_INTERVAL_MS` no reference. Minors M-1…M-9 (closed-repair "back since", stale `AssetRow.down` comment, the undeclared "expires tomorrow", predicate cleanups in e2e cases 1/2/4, `activeHoldFor` on non-IT records, no visual cue on disabled held-spare radios, a bare role comparison on the list page, the Employee/Department facet asymmetry, `holdFacets` outside the snapshot). Verified clean: migration additivity and backfill honesty, both repair-end stamps on every `LifecycleChange` path, `canAssign` byte-for-byte unchanged, e2e leftovers harmless. **Ruling R4** set the wave: I-1, I-2, I-3 (as "declare and honour day-precision `expiresAt`": `dayFloor` on the seed's two rows — blessed outside the plan's file set — a schema doc comment naming the invariant, `holdStatus.expired` through `isHoldExpired`, the dead interval constant deleted; migration 25's applied SQL untouched and no migration 26, since the only pre-existing rows are seed rows written in Manila working hours where the two rules agree), parked M-P26-8 (+M-4), M-P26-6, M-P26-7 re-scoped to M-6 (visual cue, radios stay `disabled`, no `aria-disabled` on a role-less `<label>`), M-P26-5, and the one-liners M-1/M-2/M-5/M-7. Record-only: **M-3** — `holdStatus` also says "expires tomorrow" at one day, mirroring `dueStatus` (spec §3.2's model); kept, so the pill copy is "expires in N d" / "expires tomorrow" / "expires today" / "expired N d ago". **M-9** — `holdFacets` runs its four queries after the paged snapshot and lists every employee with a hold in the tab without a `take`; correct at team scale and the shape every faceted list uses; noted, not changed. Declined: **M-P26-1** (a value import plus a re-export is the right shape when the module uses the tabs and must re-export them), **M-P26-4** (dimmed zero counts are information; the asymmetry is fixed as M-8).
- **D-9** (final-review fix wave, bbaec18 + 1442289; rulings R4 and R5) — `reserveAsset` wraps its transaction in `try`/`catch` and turns Prisma `P2002` from `Reservation_one_active_hold_per_asset` into the pinned conflict copy, re-read from the winner's row ("{tag} is already held for {employeeNo} — release that hold first."), the sibling shape of `inventory/actions.ts`; both writers store `d.reason || null` (`cleanReason` already trims). `listReservations` counts each tab over `buildHoldWhere(t.id, state)` inside the paged snapshot — the badge is the toolbar's total — and the Department facet lists every department with dimmed zero counts like the Employee facet. Day-precision `expiresAt` is declared (a `///` comment on `Reservation.expiresAt` in `schema.prisma`; no migration, migration 25's SQL untouched) and honoured by every writer: the seed's two rows through `dayFloor`, `e2e/paging.spec.ts`'s 30 scale-fixture holds through `dayFromISO(addDays(localDateISO(now), 7))` (R5), all eight `reservation.create` sites in `src/`, `e2e/` and `prisma/` verified; `holdStatus.expired` is computed by `isHoldExpired` so the one rule has its consumer; `HOLDS_SWEEP_INTERVAL_MS` (zero references) deleted; the worker's docblock says why `lt: start of today` is the calendar rule in SQL. e2e cleanups: case 6 creates inside its `try` and deletes only what exists; cases 1/2 delete by the read-back id with a `createdAt >= startedAt` fallback; case 4's net is narrowed the same way. `HoldsTable` renders the Actions header and cell only for `canRelease`. Held spares in the three loadout pickers look disabled (`cursor-not-allowed opacity-55`, no hover) through one `isHeld` helper; the radios stay `disabled` and no `aria-disabled` is put on the role-less `<label>`. `required={returnReasonRequired}`. The record's Repair card reads "down N d, closed <date>" when the status family is closed (RETIRED/SOLD/DONATED) and keeps "back since" in service; `AssetRow.down`'s comment matches `downDays`; `activeHoldFor` runs only for IT records; `/reservations` decides Release with `isDirectLifecycle(user.role, "IT")`. Verification: tsc, eslint clean; vitest 85 / 1511; `migrate status` 25 up to date; Playwright holds + paging + custody `29 passed (2.9m)`, then paging `12 passed (1.2m)`; seed last, both seeded `expiresAt` rows at `T00:00:00.000Z`. Re-review (sonnet): CLEAN — every finding addressed, nothing new; one reader's note that the catch block queries `prisma`, not the rolled-back `tx`, on purpose. Spec amendments this entry drives: §0 decision 8 (the index exists since migration `20260814090100`; the application check stays as the friendly first answer, the index as the guarantee, the catch as the translation), §3.2/§9 copy list (+"expires tomorrow"), §4.3 (day-precision storage invariant), the record copy at §6 ("closed <date>" for closed-family statuses), §7/P-8 (cleanup by id, or by the test's own `createdAt` window when the UI created the row).
- **D-10** (Task 6, the closing battery and this docs commit) — **Measured at close, on the final code tree `1442289`** (no product code or test file changed in this task): `npx tsc --noEmit` clean (exit 0, no output) · `npx eslint .` clean (exit 0, no output) · `npx vitest run` **85 test files / 1511 tests, all passed** (2.01 s) · `npx prisma migrate status` — **"25 migrations found in prisma/migrations" / "Database schema is up to date!"** against `inventory_dev` (migration 25 `20260921072537_repair_end_and_holds`, additive with its audit-history backfill; `main` and staging stay at 24 until the merge and the first `-Force` redeploy after it) · `npx playwright test --list` **`Total: 364 tests in 35 files`**, exactly the number P-7 predicted. The seven chunks then ran FOREGROUND, one at a time, `E2E_PORT=3100 --workers=1 --global-timeout=540000`, with `netstat -ano | findstr :3100 | findstr LISTENING` printing nothing before each and after the last: **A 54 passed (3.5m) · B 67 passed (3.9m) · C 63 passed (4.1m) · D 47 passed (3.6m) · E1 36 passed (5.7m) · E2 31 passed (3.0m) · F 66 passed (6.6m) = 364**, zero failed, zero flaky, zero skipped, zero did-not-run — every chunk green on its first pass, so no chunk was re-run and this phase carries no test-only battery fix commit (the third close in a row with none). Only F moved, 55 → 66, for the eleven `e2e/holds.spec.ts` cases; A, B, C, D, E1 and E2 matched Phase 25's counts exactly. `npm run db:seed` ran last, once, after the final chunk — "Seed complete." — and port 3100 was free afterwards. Documents updated in one commit: `docs/HANDOVER.md` (line 3's leading parenthetical, the new **(s)** block, §0 item 9's chunk F line, counts sentence and a Phase 26 battery paragraph, §4's `/reservations` route bullet, Phase 25's (r) "what is parked" paragraph rewritten as delivered, and five ✅ CLOSED in Phase 26 marks plus two re-worded still-true notes in §8), `docs/PICKUP.md` (Branch, Database, a new Battery row, Last two phases 26/25, §4's new item 1 with the list renumbered, §5's Phase 26 bullet closed), `docs/HANDOVER-PENDING.md` §6 (both items "Shipped in Phase 26"), this plan's D-block, and the spec's Status line with seven `*Amended (D-n):*` notes.
