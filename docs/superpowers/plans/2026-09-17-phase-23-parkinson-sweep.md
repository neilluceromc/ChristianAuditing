...
## Amendments made during execution (`D-1`…`D-20`, from the SDD ledger, 2026-09-17)

Pre-flight and early rulings:

- **D-1** (ruling R1) — spec §9.1's employee-schema unit test was dropped: `employeeSchema` is module-private in a `"use server"` file and cannot be exported; the date floor and the blank-department refusal are covered by e2e `deadlines.spec.ts` cases 2 and 7.
- **D-2** (ruling R2, spec §4.2 amended) — the spec's "every chip ≥ 5 characters" rule was over-strict: every chip site's minimum is 3 (`reasonRequired()` defaults to 3, the stock adjust/write-off schemas use `min: 3`); the only min-5 field, `returnAssetToIt`, has no chips. The approved wording "Lost" stays in `stock.write-off` and `asset.missing`; `asset.assign`'s third chip reads "On loan"; the unit test's floor is 4. The Task 2 implementer had first widened the words to satisfy the literal rule; the ruling restored them (commit `b5393c7`).
- **D-3** (measured rendering) — `fmtDate` renders September as "Sept" (en-GB CLDR), so `deadlines.test.ts` expects "14 Sept 2026" where the plan's test text wrote "Sep". Measured output governs.
- **D-4** (ruling R3, spec §7 amended) — the date inputs carry `min`, so a past date is refused by the browser's native constraint validation and the form never submits; the server floors ("Pick today or later", "Pick tomorrow or later") are unreachable through the UI by design. The e2e strips `min` via `evaluate` before submitting and asserts the server text — the guard the spec cares about. Both layers stay.
- **D-5** (process) — `git commit -- <paths>` skips untracked files: Task 2's first commit (`b83d5c9`) carried only the modified files and a second commit (`958840e`) added the six new modules; every later brief says `git add` new files first.

Task rulings:

- **D-6** (Task 1) — migration 24 `parkinson_deadlines` landed as spec §2.4 verbatim (two `ADD COLUMN`, two `IS NULL`-guarded backfills, `SET NOT NULL` after), applied on `inventory_dev` (24); seed: Dennis Ong `EMP-0090` due two days back, `ST-0001.dueAt = day(-8)`, no OPEN stocktake seeded (an open one would block every test that opens one). Review clean (0 Critical / 0 Important / 1 Minor: a doc-comment wording nit). Commit `8d211aa`. Known interim: `tsc` failed in `stocktake-actions.ts` until Task 4 added `dueAt`.
- **D-7** (Task 2) — the pure rules landed tests-first in three commits (`b83d5c9`, `958840e`, `b5393c7`): `deadlines.ts` (working days Mon–Fri, defaults 5 wd / 3 d, floors today / tomorrow, `dueStatus`, `isPastDue`, `offboardingCompletionText`), `reason-chips.ts` (+ `chipsForOutcome`), `recent-picks.ts`, `leaverRow` (rank 2, severity 1000 / 500 − days / 2 / 1 / 0), the offboarding `due` facet and sort, `stocktakeOpenSchema.dueAt`. 82 unit files / 1476 tests (+3 files). Review: spec ✅ incl. R2 and D-3; Approved; 0/0/2 Minors parked.
- **D-8** (Task 3) — `ReasonField`, `DuePill`, the `EntityCombobox` Recent/All groups with `autoFocus` (P-3), the focus trap's first-form-control preference (P-4) and `src/server/recent-picks.ts` (P-1) landed in one commit `e849460`. Review: spec ✅ (one note: `ReasonField` omits the spec's optional `id`/`aria-label` props — P-10 makes them unnecessary; spec §6.1 amended accordingly); Approved; 0/0/2 Minors parked.
- **D-9** (Task 4) — the server layer landed in one commit `b5e87fd` (11 files): `updateEmployee`/`createEmployee`/the importer resolve `offboardingDueAt` on every OFFBOARDING entry; `openStocktake` floors and stores `dueAt` and audits it; `rememberPicks` after commit at the seven sites; `leaverRow` on Home; `OffboardingRow.dueAt`, the `due` facet and filter, `WizardData.employee.dueAt`/`completedAt`; `stockHomeSignals.stocktakesOverdue`; `findSameSupplierName`/`checkSameSupplierName`. Both DB proofs reproduced by the reviewer (opus): `stocktakesOverdue 0`; Dennis due −2 d; `facets.due` overdue 1 / on-track 0. Review Approved; 0/0/5 Minors (comment and polish) parked.
- **D-10** (Task 7, dispatched early) — Task 7 consumed only Task 2/3 exports, so it started before Task 4 finished; its file set was disjoint. `inventory-table.tsx` was edited beyond the brief's list to thread `recentEmployees` from the inventory page to `BulkDrawer` (the brief said "the page that renders `<BulkDrawer>`"; the table component is where it renders) — accepted. Commit `2faf841` (18 files). Review: spec ✅ 18/18; Approved; 0/0/1 Minor (chips not disabled while pending in the dialogs) parked.
- **D-11** (Task 6) — commit `dd40213` (10 files incl. the new `supplier-same-name.tsx`). A leftover `next-server` child on 3101 needed a force kill after the curl check (the known Windows reaping quirk). Review: spec ✅ 10/10; Approved; 0/0/1 Minor (issue-form Purpose chips not disabled while pending — same class as D-10's) parked.
- **D-12** (Task 5) — commit `54b90a7` (10 files); the foreground walk on 3100 confirmed every contract point on the seeded data (Dennis "2 d overdue" on the list, facet, wizard and report; the employee form's date field; "Choose a department"; `/inventory/work`; the stocktake "Close by" prefill and header pill; the Purchasing tile at 0). The walk opened `ST-0002` to see the OPEN state and cancelled it afterwards. Review: spec ✅ 10/10 with the accessible names verbatim; Approved; 0 Critical / 0 Important / 0 Minor.
- **D-13** (Task 8, ruling R4, spec §9.2 and §6.3 amended) — `deadlines.spec.ts` (8 cases) landed green twice, foreground: **8 passed (1.1m)** both runs; `--list` **327 tests / 32 files** (319 + 8, 31 + 1). The spec named an `/employees/<id>/history` route that does not exist — employees have a Timeline (actor + action) and only `/audit` renders diff field names — so case 2 asserts the newest Dennis Ong row on `/audit?entity=employee` shows `offboardingDueAt` + `update` plus a DB poll; the diff's old → new values are not rendered for employees and are not asserted. No employee History tab is owed in this phase. Both date floors are exercised per D-4 (R3). Leo Tan is restored to ACTIVE in a `finally`; the backdated stocktake is cancelled through Prisma (P-7). Commit `bd794d8`. Review: spec ✅ 8/8 (every locator, text and route cross-checked against source); Approved; 0 Critical / 0 Important / 2 Minor (style) parked.
- **D-14** (Task 9) — `quick-forms.spec.ts` (9 cases) landed green twice, foreground: **9 passed (1.1m)** both runs; `--list` **336 tests / 33 files** (327 + 9, 32 + 1). Dennis Ong is OFFBOARDING and the issue form lists ACTIVE employees only, so case 1 picks the ACTIVE `EMP-0097` and derives its match set from the database through the combobox's own filter rule. The task surfaced the branch's one Important: `ReasonField`'s chip group (`aria-label="Quick reasons"`) and chips (`aria-label="Use reason: <chip>"`) collide with Playwright's `getByLabel("Reason")`/`("Purpose")` — a case-insensitive substring match over `aria-label` — at 21 call sites in 13 pre-existing spec files, and four `selectOption` calls (`stock.spec.ts`, `stock-lots.spec.ts`, `registration.spec.ts`) now target comboboxes. **Ruling R6** (fix wave): the group reads "Quick picks", chips carry no `aria-label` (their visible text is their name), the four `selectOption` sites use the suite's combobox idiom — product-side naming rather than 13 edited spec files. Commit `a16ad8c`. Review: spec ✅ 9/9 (every case verified against source and seed); Approved; 0 Critical / 0 Important / 2 Minor (robustness notes) parked.
- **D-15** (final review, opus) — Approve with fixes: 1 Critical, 2 Important, 11 Minor on `8b2dfd7..a16ad8c` (71 files). Critical: spec §6.1/§8's `enterKeyHint` never reached the plan or any brief, so no task review could see it. Important 1: `ReasonField`'s chip `aria-label`s collide with `getByLabel("Reason")`/`("Purpose")` (D-14, R6) plus four `selectOption` sites now on comboboxes. Important 2: an overdue leaver's employee form could not be saved at all — the date input's `min={today}` blocked the stored past date natively and the server floor was unconditional (seeded by Dennis; guaranteed on staging by the backfill). Minors: chips not disabled while pending (R5), Task 4's four polish items, the seed storing the two new dates at wall-clock time (a phantom audit diff on first save), `updateEmployee` not revalidating the offboarding pages and Home, a render-phase ref write in `receive-form.tsx` (the Quantity id captured inside the render prop), and six deferred (tile accent tone, "No supplier" clear on the receive combobox, the dropdown auto-opening on `autoFocus`, facet-count coupling, the OFFBOARDED wizard pill, two §5.4/§8 wording reconciliations).
- **D-16** (ruling R7, spec §6.1 and §8 amended) — `enterKeyHint` is implemented once as a default in the shared `Input` component (typeable types render `enterKeyHint="next"` unless the caller passes one); the "done on a dialog's last field" nuance is dropped. Fix wave, commit `27479ad`.
- **D-17** (ruling R8, spec §7 amended) — the employee form's date `min` is the earlier of today and the stored value, and `updateEmployee` applies "Pick today or later" only when the provided date differs from the stored one; a regression case in `deadlines.spec.ts` (case 9) saves Dennis's record (2 d overdue) with a Title change only. Fix wave, commits `27479ad` (product) and `dbc93a0` (e2e).
- **D-18** (ruling R6 landed) — `ReasonField`'s group reads "Quick picks" and chips carry no `aria-label`; `quick-forms.spec.ts` locates chips through a "Quick picks"-group-scoped helper; the four `selectOption` sites use the combobox idiom scoped to the input's own listbox (the Item combobox's `autoFocus` leaves a second listbox open on the issue and receive pages). Fix wave, commits `27479ad` and `dbc93a0`; the affected e2e files re-ran green — quick-forms + stock 20 (2.6m), stock-lots + registration 15 (1.8m), deadlines 9 (1.5m).
- **D-19** (ruling R5 landed, Task 4 Minors, seed, revalidation) — `disabled={pending}` at the seventeen `ReasonField` sites that lacked it; `offboardingFacets`' comment and `derivedCandidates` rename; `completedAt` inside `getWizard`'s `Promise.all`; the dead `updatedAt` select dropped; the `updateAsset` no-op comment; the seed's two new dates floored to the Manila calendar day (`dayFromISO(localDateISO(d))`, so a seed run between 00:00 and 08:00 Manila keeps Dennis at exactly 2 d overdue — the ruling accepted this over the brief's literal UTC-day example); `updateEmployee` revalidates `/offboarding`, the wizard and report pages, `/inventory/work` and `/`; and (ruling R9, final-review Minor 10) the receive form's Quantity focus uses a forwarded ref instead of a render-phase ref write. `/` was not added to `revalidateStocktake` (inert today; parked). Fix wave, commit `27479ad`. Scoped re-review (`a16ad8c..dbc93a0`): all eight findings ADDRESSED, no new breakage; tsc, eslint and vitest re-confirmed clean.
- **D-20** — Measured at close, 2026-09-17, final tree `09bf0a5` (product tree `0fe8b04`; 14 commits above `main` at `8b2dfd7`, the docs commit that records this on top): `npx tsc --noEmit` clean · `npx eslint .` clean · `npx vitest run` **1476 tests / 82 files** passed · `npx prisma migrate status` **24 migrations found, schema up to date** · `npx playwright test --list` **337 tests in 33 files**, the same before the chunks and after. Seven foreground chunks, one process at a time, `E2E_PORT=3100 --workers=1 --global-timeout=540000`, port 3100 with no LISTENING process before and after each — Phase 22's seven, with `e2e/quick-forms.spec.ts` joining chunk D and `e2e/deadlines.spec.ts` joining chunk F: **A 54 passed (3.4m) · B 67 passed (5.8m) · C 63 passed (5.9m) · D 47 passed (4.7m) · E1 36 passed (8.9m) · E2 31 passed (4.1m) · F 39 passed (4.8m) = 337**, zero failed, zero did-not-run. `npm run db:seed` ran last, clean, and the port was free after it.

  One test-only fix, its own commit `09bf0a5` (`e2e/department-owned.spec.ts`, +6 lines, no product code). Chunk B's first pass on `dbc93a0` was **64 passed, 2 failed, 1 did not run**, and an immediate as-is re-run reproduced both failures identically. They are one bug. Case 9's second create — the case-variant duplicate — never picked a department, relying on the form's pre-Phase-23 auto-default (`departmentId: departments[0]?.id`); spec §6.2 deliberately replaces that with `departmentId: ""` and a `<option value="">Choose a department</option>`, so the create is now refused on "Pick a department" before it can reach the duplicate-employee-number check the case exists to prove (the failure snapshot shows `combobox "Department" [invalid]` and `alert: Pick a department`). Because case 9 sits in a `test.describe.serial`, its failure retires the Playwright worker: the file's own `beforeAll` `npm run db:seed` runs again before case 11, wiping the approvals cases 2 and 4 leave PENDING. Purchasing Home then reads Approvals 1 / Below reorder level 1 / Expiring within 30 days 1 — measured straight from a fresh seed — and case 11's `getByRole("link", { name: "1", exact: true })` matched three stat links instead of one. Diagnosis confirmed by re-running cases 1–11 alone and reading the approval table back (only the three seeded rows survived). The fix adds `await page.getByLabel("Department").selectOption({ label: "IT" });` before the duplicate submit, with the reason in a comment; chunk B then measured **67 passed (5.8m)**. Chunk A was re-run on the final tree afterwards, so all seven numbers above are post-fix. **Worth carrying forward: a failing case inside `describe.serial` re-runs the file's `beforeAll` for whatever follows it, so the next case can fail on a freshly reseeded database rather than on anything of its own.**
