...
- **D-1 (R1) The worktree branch was fast-forwarded to local `main` `1acd928`** (`git merge --ff-only main`), so the spec (`401b838`) and the plan (`1acd928`) are in the branch's history, as in Phases 20–28. Why: every phase since 20 has branched from local `main`, and the final merge is `--no-ff` back into the same `main`. Cost if wrong: none.
- **D-2 (R2) `LoadoutView`'s inert-tile branch is `!canMutate` (a viewer), not the plan's `!mayAct && !frozen`.** A frozen profile seen by IT keeps tile buttons whose menu is just `["open"]`, and the ⋯ renders whenever `canMutate`. Why: spec §4.4 makes viewer tiles inert in every state; the plan's condition leaked a working menu to viewers of leavers. Cost if wrong: one condition.
- **D-3 (R3) The same-name refusal splits across title and body.** `_sameName` reads “Not added — tick ‘This is a different person’ to add them anyway” as the banner's body line; the title keeps “Another {name} exists in {department} ({no})” with the number linked. Why: P-3's full sentence would have repeated the title word for word, and `directory.spec.ts:84`'s pinned substring survives either way. Cost if wrong: one string.
- **D-4 (R4) `custody.spec.ts` case 8 adapts in Task 7** — the progress line to “N of M slots filled” (spec §4.2) and the menu label to “Waive this slot…” (spec §4.4). Why: the spec's pinned copy is binding and the plan's Task 7 wrongly predicted no `custody.spec.ts` change. Cost if wrong: two test lines.
- **D-5 (R5) Menu items whose target is a route handler navigate with `window.location.assign(href)`** — “Export holdings” (`/employees/[id]/holdings`) and the list's “Export” (`/employees/export`). Why: `router.push` into a route handler is a client navigation into a download; `ButtonLink` is a plain anchor today. Cost if wrong: two lines.
- **D-6 (R6) An ACTIVE person under an all-optional policy with nothing assigned gets NO primary** (`profilePrimary` returns `null`). Why: `missingRequired` is 0 and spec §4.4 calls optional slots “not a gap”, so “Assign kit” would demand what the policy does not. Cost if wrong: one condition in `profilePrimary`.
- **D-7 (R7) `SpareOption.pendingRef` is computed from every open approval on the asset**, not only the ones belonging to this employee, and the Reserve picker honours it alongside Fill and Replace. First deferred as pre-existing data scope (the old radiogroups saw the same data), then reopened as the final review's I-1 and fixed in the wave. Why: a spare queued for someone else must not be offered, or the approval fails at execution. Cost if wrong: an IT operator picks a spare another person's approval is waiting on.
- **D-8 (R8) One reservation marks ONE tile.** A single consuming pass over the ordered tiles attaches each reserved spare to at most the first empty matching tile, replacing the per-tile `find` on `typeId`. Why: spec §4.6 puts a reservation on “the tile”, singular; the plan-mandated find lit every same-type tile with its own Assign reserved button. Cost if wrong: one tile fewer shows the hold.
- **D-9 (R9) The taken-number message shows the number as typed** (“{no} is already {name}'s”), not uppercased as the plan's Step 5 prose implied. Why: the operator should see what they typed. Cost if wrong: one `.toUpperCase()`.
- **D-10 (R10) The submit intent comes from the DOM submitter**, `(e.nativeEvent as SubmitEvent).submitter` with `name="intent"` / `value` on the two buttons; the `addAnother` React state is gone. Why: the review's Critical showed the flag set in a button's `onClick` could be read stale by the form's `onSubmit` in the same task; the plan's own fallback clause allowed this shape. Cost if wrong: two attributes and a submitter read.
- **D-11 (R11) “No items yet” must not swallow the whole Stat grid.** A person holding nothing was losing the Open requests count even with an open request (Nina Robles + APR-2041). Found by the Task 7 e2e, marked PROVISIONAL in the `it-nav` adaptation, fixed in the final wave together with the restored assertion. Why: spec §4.2. Cost if wrong: one Stat on a day-one profile.
- **D-12 (R12) The e2e fixture restore stays a database write in `finally`, and case 4 additionally drives a real Return through the tile menu to submission in its body** (asserting the toast, the emptied tile and the primary flipping back to “Assign kit”). Why: a `finally` that drives the UI is fragile when the page is already mid-failure, but the phase's new trigger path must still be exercised end to end. Cost if wrong: one e2e case body.
- **D-13 (R13) `tileMenuItems` keeps offering “Waive this slot…” on any policy slot without an exception, optional ones included**, and the now-dead `required` parameter is removed from the signature and the tests. Why: this is pre-phase behaviour, and spec §4.4's “when the slot is required” reads as “a policy slot”. Cost if wrong: one menu item on optional tiles.
- **D-14 (R14) Where the plan and the spec disagreed about the profile's Stats, the spec won.** Task 3 Step 3 said “replaces the four Stats”; spec §4.2 says “No items yet” replaces Book value and Oldest item only — Items held is implied by the line and Open requests stays whenever `openApprovals.length > 0`. Root cause of D-11. Cost if wrong: one Stat.
- **D-15 (R15) The four read-only live checks meter against their own `check` rate kind at 240/min**, not the 60/min mutation budget. `RateEvent.kind` is a plain `String`, so there is still **no migration**. Why: a bulk-add burst of number and policy checks could otherwise starve the create button. Cost if wrong: a second rate-row kind.
- **D-16 (R16) The list's search input keeps `key={state.q}`, and losing focus after Enter is parked, not fixed.** The remount is what clears the box when “Clear search” empties the URL, and it also fires on Enter. Why: after a search the operator's next target is the result, and the same `defaultValue` + `key` shape exists in five sibling toolbars, so the ref-clear rewrite belongs in one post-merge toolbar commit. Cost if wrong: one Tab press after Enter.
- **Measured at close (final tree `9660e6e`, 2026-09-22).** `tsc` clean · `eslint` clean · `npx prisma migrate status` **26 migrations**, schema up to date (this phase adds none) · `npx vitest run` **1556 tests / 90 files** · `npx playwright test --list` **389 tests in 38 files**. The seven foreground chunks, `E2E_PORT=3100 --workers=1 --global-timeout=540000`, composition unchanged from Phase 28 apart from `e2e/employee-ux.spec.ts` joining **E1** (P-13, spec decision 9): **A 54 passed (3.4m) · B 67 passed (4.1m) · C 64 passed (4.2m) · D 50 passed (3.7m) · E1 46 passed (6.6m) · E2 42 passed (4.1m) · F 66 passed (6.8m) = 389**, zero failed, zero skipped, zero did-not-run. E1 moved 37 → 46; no other chunk's count changed. One flake and no fix commit for it: chunk B's first pass was 66 passed / 1 failed (4.4m) on `e2e/asset-classes.spec.ts:499` (“21. names Purchasing, points at the Register screen, and admin can follow the fix”), and the as-is re-run was 67 passed (4.1m). `npm run db:seed` ran last, clean — it also removed the throwaway rows EMP-9801 / EMP-9802 (“Throwaway Fixround”) a Task 5 fix-round click-through had created in `inventory_dev`. Port 3100 carried no listening process before the first chunk, between any two chunks, or after the last. The phase is CODE-COMPLETE, UNMERGED and UNPUSHED; merging (`--no-ff`, verified by tree hash because the root checkout has no `node_modules`), pushing and the staging redeploy are the user's decisions.
- **Post-merge hygiene** — recorded by the final review and its re-review, none of it blocking: extract `SlotTileCard` and `useTileFeedback` out of the ~1070-line `src/components/employees/loadout-view.tsx`; give `EmployeeNoCheck`, `PolicyPreview` and `SameNameCheck` one shared `useDebouncedCheck` hook instead of three near-identical debounce/staleness checkers; fix the five sibling toolbars' uncontrolled search inputs together (the `defaultValue` + `key` shape behind D-16); drop or re-scope `rateLimited()`'s “60 changes” sentence, which is unreachable once the refusal comes from the new `check` kind; and reconcile `reservationsBySlot`'s consuming pass with `reservedCount`, which still counts every hold, so a loan-covered empty tile can read “on loan”, “reserved · tag” and offer Assign reserved at once.
