...
- **D-1 (R1) The branch starts at local `main` `b582535`.** The worktree was fast-forwarded (`git merge --ff-only main`) so the spec `a94f47a` and this plan are in its history, as in Phases 20–29. Cost if wrong: none — the merge is `--no-ff` into the same `main`.
- **D-2 (R2) The three list-header pins move to Task 6.** `asset-classes.spec.ts:435`, `it-core.spec.ts:152` and `registration.spec.ts:215` were adapted in Task 6, not Task 5: during Task 5 the old New asset link still reached Register through the redirect, so they kept passing. Cost if wrong: three test lines move between tasks.
- **D-3 (R3) Quantity-1 errors land on row 1.** The merged form submits quantity 1 through `createAsset`, whose error keys are `tag` / `serial`; the form maps them onto row 1's cells (`Row 1 · …`) so both paths render errors in the same place. Cost if wrong: one mapping function.
- **D-4 (R4) Loan at quantity 1 only on the direct path.** An approval-path registrant (e.g. purchasing_staff registering IT) is not offered Loan, because the approval executor never applies `loanDueAt` and would create a dateless loan (spec §5.3); Loan shows only when `isDirectLifecycle(role, cls)`. Cost if wrong: such a device is registered Deployed and IT converts it to a loan.
- **D-5 (R5) `it-gaps.spec.ts` case 4 moves to Task 3.** Task 2's Replace exclusion broke it (BR-LT-0181 was BR-LT-0201's only same-type spare); Task 3, which wires `hidden` into the Replace dialog, adapted it. Cost if wrong: one e2e case moves between tasks.
- **D-6 (R6) Change status on the approval path offers only `statusTargets`.** The executor's `apply.ts` guard refuses the same targets, so hiding them prevents requests that would fail at execution; direct-lifecycle 9 and asset-classes 6, 7, 14 adapted (14 files its approval through Prisma). Cost if wrong: one line in `status-control.tsx` restores the full list.
- **D-7 (R7) A transitional create form for one task.** `src/app/(app)/inventory/new/new-asset-form.tsx`, a copy of the old create form, kept `/inventory/new` alive between Tasks 4 and 5; Task 5 deleted it with the redirect. Cost if wrong: one duplicated component for one task.
- **D-8 (R8) Wrong-class record URLs read “Asset not found”.** After the route-group move `[id]/not-found.tsx` catches wrong-class record and edit URLs instead of the app-level “This page doesn't exist”; accepted — the asset is absent from that role's view and the copy reveals nothing more; three e2e assertions updated. Cost if wrong: one not-found message.
- **D-9 (R9) Save and Cancel leave Edit the way Back does.** `leaveFor` in `nav-stack.ts` (reusing `popForBack`) pops the record entry and calls `router.back()` when the previous entry is `/inventory/{id}`, else `router.replace(record)`, so the record's Back still returns to the list (Phase 28's promise); back.spec case 4 covers Cancel and Save. Cost if wrong: two call sites.
- **D-10 (R10) A single registration shows its Last change line.** Merging the create flows routes quantity 1 through `createAsset`, which audits `create`; `lastLifecycleChange` also matches `create`, so the record reads “Last change: created · {date} · {actor}”. Cost if wrong: one OR term.
- **D-11 (R11) Staged documents carry across the quantity switch.** `carryDocuments` in `src/lib/register-documents.ts` (6 unit tests): 1 → N makes the first file the batch invoice and keeps the others listed with a warning; N → 1 returns the invoice with kind Invoice. Found by Task 5's review — files were silently lost. Cost if wrong: one helper.
- **D-12 (R12) The Register link carries the viewed class.** The list's header and empty-state link is `/inventory/register?cls={cls}`, not `withViewClsQS`, because the Register page narrows categories only when `?cls=` is present — purchasing staff otherwise saw both classes' categories. Cost if wrong: one href.
- **D-13 (R13) A clipped bottom-row menu flips upward.** Measured: the last row's menu spanned 682.9–804.9 against a wrapper bottom of 683.9; Menu flipped upward when it would overflow its scroll container. Superseded by D-14.
- **D-14 (R14) Menus render in a portal.** Task 8's two-row list case showed a menu can fit neither side of a short table; Menu now portals its popup (to `document.body`, or the open Dialog/Drawer's root), fixed-positioned from the trigger, flipping against the viewport only, repositioning on scroll/resize, with keyboard open, Tab / Shift+Tab and `aria-controls` (Task 7 rounds 2–3). Cost if wrong: one primitive's positioning.
- **D-15 (R15) The fix wave.** The final review's four Importants — `checkIdentifiers` refuses a class the caller cannot register; the Attention pass only as the primary key; Finance confirms with `Confirm details`; pagination marks the list busy — plus the sr-only Status separator, a 24 px Clear ×, `[id]/edit/loading.tsx`, a caught Reveal, `Math.max(0, hidden)`, and a hand-crafted-only refusal of `TEMPORARY` in `createAsset` off the direct path. Everything else in the triage ships. Cost if wrong: small, local edits.
- **D-16 (R16) Three nits ship.** The double-encoded dash in a `pagination.tsx` comment, the inventory page links' lost `next/link` prefetch (I-4's trade-off), and the sr-only “ · ” some readers voice as “middle dot”. Cost if wrong: nothing at runtime.
- **Measured at close** (final tree `4e6fc7f`, 2026-09-23): `tsc` and `eslint` clean; **1624 unit / 95 files**; **26 migrations**, up to date (none added); `--list` **409 tests / 39 files**; the seven foreground chunks A 58 (3.7m) · B 67 (4.1m) · C 64 (4.2m) · D 52 (3.9m) · E1 46 (6.6m) · E2 56 (4.9m, with `e2e/inventory-ux.spec.ts`) · F 66 (6.8m) = **409 passed**, zero failed, zero flaky, no re-run; `npm run db:seed` last; port 3100 free throughout.
- **Ships knowingly:** the Attention sort is an in-memory candidate pass (revisit past a few thousand assets); Export ignores the Attention sort; `import/asset-actions.ts`'s `revalidatePath` patterns omit the route groups (verify); Phase 29's Loadout sort (`src/server/modules/employees/queries.ts:71`) has I-2's secondary-key pattern; D-8's not-found copy; the lost `next/link` prefetch and the `pagination.tsx` comment mojibake (D-16); parked — the History/Timeline merge, Record RMA / Record quote dialogs, a record-shaped list skeleton, Serial and Loan due as optional columns (`HANDOVER-PENDING.md` §5.3).
