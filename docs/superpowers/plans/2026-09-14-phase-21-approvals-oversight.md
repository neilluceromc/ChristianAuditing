...
- **D-14** (final review, process) — the whole-branch review was first dispatched on the most capable model and terminated at once on a usage limit (HTTP 429, "Fable limit reached") before reading anything; it was re-dispatched on Opus, which produced the Approve-with-fixes verdict D-11…D-13 record. The fix wave's scoped re-review found both Important findings addressed and nothing new. Two implementers walking dev servers at the same time (Tasks 2 and 3) collided once in the shared Browser pane; the rule since is one walk at a time, and `npm run dev -- -p 3100`, never the bare script (which binds the staging stack's port 3000).
