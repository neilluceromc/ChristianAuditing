...
Header line: Phase 13 code-complete on `phase-13-asset-classes`, unmerged, unpushed, with the battery numbers you actually got. **Stale counts D-19 found, fix in the same pass:** lines ~257 and ~364 say "147 tests across 12 files"; the four-command split at ~265-268 names only 12 files (it runs neither `receiving.spec.ts` nor `asset-classes.spec.ts`) — replace it with this plan's five-part split; §T13 (~586) says `import-export.spec.ts` has 20 tests (21); ~517 says `labels.spec.ts` 9 tests (15); ~1420 says `auth-shell` 15 (16). Current tree: **187 / 14 files = 52 · 47 · 59 · 23 · 6**. §0 item 4: a Phase 13 paragraph pointing at the spec and this plan, naming the two premise-level facts (own vocabulary; one register), the two accepted defaults (approvers unchanged; categories admin/IT-created), and the two triggers. §4: a "Phase 13" paragraph in the DONE list.
...
Baseline before this phase: **167 e2e / 13 files** (52 · 47 · 39 · 23 · 6). Expect **187 / 14 files** — 167 + 20 (D-8, D-13, D-14, D-15, D-17; case 15 dropped, cases 20 and 21 added). **Read every count; write down what you got.** **GOT (2026-09-07, controller, foreground, each part its own call):** tsc clean · lint clean · vitest 908 / 51 · `npm run build` compiled · compose prod image rebuilt by the interrupted battery agent (timestamp only; not repeated) · e2e **52 (3.1m) · 47 (5.2m) · 59 (6.8m, 58/59 on first run — `purchases.spec.ts:90` bounce-back centrepiece; passed alone in 34.5s and 59/59 on the repeat, 6.2m — a non-reproduced headroom-class failure, not a Phase 13 regression) · 23 (2.0m) · 6 (3.9m) = 187 / 14.** Axe moderate, counted not failed: `empty-table-header` 10 · `page-has-heading-one` 2 · `landmark-unique` 1 (the class switch's second `navigation` landmark). The battery agent itself died with the session mid part 1; the controller re-ran the static half and all five parts — the rule from [[subagent-model-selection]]: long mechanical runs belong in controller foreground calls. ⚠️ `home-finance.spec.ts`, `it-core.spec.ts` and `import-export.spec.ts` are the ones to watch: they assert IT-side counts and totals, and a failure there means a query was not pinned to `cls: "IT"` (Task 9 Step 3) — fix the query, never the assertion. **That heuristic does not cover `financeHome` (D-16): nothing asserts Finance Home's Capitalized tile. Add an assertion in `home-finance.spec.ts` that it totals BOTH classes' seeded costs — **₱48,442,000 across 32 assets** (IT ₱807,000 / 25 + Purchasing ₱47,635,000 / 7; D-19 corrected D-16's "₱47.6M", which was the Purchasing sum alone) — derived in the test from `db.asset.aggregate({ where: { cost: { not: null } }, _sum: { cost: true }, _count: { _all: true } })` rather than typed, so the one unpinned query is pinned the other way and survives the next seed edit.** Also record in §8 (D-19): `labels.spec.ts` now prints 4 Purchasing labels of 13 (its numbers hold; the meaning moved); Home's leaver card pins IT holdings while the offboarding queue does not; no test drives a Purchasing approval through the real approve action.

- [x] **Step 5: Finish the branch** — `superpowers:finishing-a-development-branch`. **Merging and pushing are the user's decisions, separately.** Done 2026-09-07: the user chose "also push to github" — merged `--no-ff` as `6bcf4e8` (byte-identical to the branch tip; tsc + 908 unit re-run on `main`), secret scan clean, `main` pushed (`b6aa5e2..6bcf4e8`). The phase branch is kept locally.
