...
**Status:** design approved in conversation 2026-09-14 (the approvals-visibility cluster from the post-Phase-20
IT survey; the shape "oversight of direct changes"; the stored-flag approach), spec written the same day.
The user asked to run the whole method through to the finishing menu without further check-ins; merging,
pushing and redeploying stay the user's decisions.
