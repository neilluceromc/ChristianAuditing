...
3. **Start offboarding is a button with a confirm dialog**, not a one-click: the Complete-by date is prefilled to the Phase 23 default (start + 7 days) and editable, mirroring Phase 23's "defaults, editable at start" ruling. Starting an offboarding stays a **direct** employee edit (no approval), as it is today through the Edit form.
...
`WizardItem.blockedBy` gains `id: string` (`{ id, refNo, type }`) and the `Decision` the collect step renders exposes the approval `id` it already derives from (`decisionOf` reads approval rows — carry `id` onto `Decision`). The three `<Link href="/approvals">` at the collect step (decided ref and blocked ref) and the summary become `href={`/approvals/${id}`}`. The `items` prop passed to the scan pool is unchanged (it carries `blockedBy: refNo | null` for the verdict text only).

### 3.4 Wizard header due pill after completion

`page.tsx` header: `employee.dueAt ? <DuePill dueAt={employee.dueAt} today={today} withDate tone={employee.employment === "OFFBOARDED" ? "closed" : undefined} /> : …` — if `DuePill` has no `tone` override today, add an optional `tone` prop that, when given, replaces the derived overdue/on-track tone (a completed case is neither). The report page already prints the date; nothing else changes.
...
| 3 | Employee timeline (`employees/[id]/timeline/page.tsx:73-74, 85`; query `employees/queries.ts:307`) | approvals select adds `id` on the row and `asset: { select: { id, tag, model } }`; reservations select adds `asset.id` | `refNo` → link to the request; approval-row `asset.tag` and hold-row `r.asset.tag` → `<Link href={`/inventory/${asset.id}`}>` |
| 4 | Activity feeds (`components/patterns/activity-feed.tsx`; pages `inventory/activity`, `employees/activity`, Home feed) | `ActivityItem` gains `entity?: { label: string; href: string | null }`; the pages fill it from the `entityLabels` map they already build | after the sentence, when `entity?.href`: `<Link href className="shrink-0 font-mono text-[10.5px] text-accent hover:underline">{entity.label}</Link>`; when `href === null` (deleted entity) nothing extra renders |
...
1. Start offboarding: as IT on a seeded ACTIVE employee the button is present; as viewer it is absent; dialog shows Complete-by prefilled to today + 7; submit lands on `/offboarding/{id}`; the employee reads OFFBOARDING with `offboardingDueAt` = that date; one `update` audit row with the three fields; back on the profile the button is gone and the frozen banner shows the due pill. `finally`: restore `ACTIVE`, null dates, delete the audit row.
2. Start offboarding validation: with `min` stripped, a past date → "Pick today or later" under the field; employment unchanged.
3. Wizard: a seeded decided item's ref links to `/approvals/{id}` (href asserted, then followed).
...
12. Offboarding: facet counts narrow each other (`due=overdue` changes the progress counts); Export downloads an `.xlsx` whose row count equals the queue total; the completed leaver's wizard header shows the due pill.
13. Audit: a department create/rename entry shows the department name (seed one via the admin page if none is seeded; `finally` deletes it).
