...
**Status:** implemented on `phase-30-inventory-uiux` (plan `docs/superpowers/plans/2026-09-23-phase-30-inventory-ux.md`, final tree `4e6fc7f`, 2026-09-23); merged to `main` via `--no-ff` `69f8545` and pushed 2026-09-24 (the merged tree is byte-identical to the tested tip `bba0f6c`); deployed to staging by the 2026-09-24 code-only `-Force` redeploy (`e07bef7`); no migration. Amendments made during execution are marked *Amended (P-n / R-n)* below and listed in the plan's D-block.
