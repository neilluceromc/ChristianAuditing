...
**Status:** **implemented on branch `phase-26-holds-and-repair-end`** (6 tasks, `D-1`…`D-10` — the spec corrections before the plan are `D-2`, the final whole-branch review added `D-8`, the fix wave `D-9`, the final battery `D-10`), **code-complete 2026-09-21 at final tree `1442289`**, the final-review fix wave included; **merged to `main` via `--no-ff` `a5883ad` and pushed 2026-09-22** (the merged tree is byte-identical to the tested tip `6175af3`); **staging redeployed 2026-09-22 with `-Force`** — **migration 25 `repair_end_and_holds` applied** (25 found, all applied; the backfill filled `repairEndedAt` on one of staging's two closed repairs), web healthy, local and LAN `/login` 200. Design approved in conversation 2026-09-21 (approach A — direct IT actions, one nullable column, an hourly worker sweep; decisions 1–8 below); the `*Amended (D-n):*` notes below record where execution changed it.
...
The layout loads `hold = await activeHoldFor(asset.id)` and passes `today`. Predicates: `canReserve = direct && !pending && !asset.assignee && isAssignable(asset) && !hold`; `canReleaseHold = direct && !!hold`. `canAssign` is UNCHANGED — Assign stays available on a held asset (an existing e2e assigns the holder their held spare), only Reserve hides while held.
