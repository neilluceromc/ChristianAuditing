...
1. **Phase 28 — a Back control on every page that has a parent: where the operator came
   from when this tab has a previous in-app page, otherwise the breadcrumb's parent — IS
   CODE-COMPLETE on `phase-28-back-control` at final tree `5a8bbcf` (product commit `ad39271` plus
   the e2e fix round `5a8bbcf`, with one docs commit above it), and is **UNMERGED and UNPUSHED**.**
   ...
   adapts) and what stays parked. **The finishing menu is the user's to choose, none of it
   pre-authorised:** merge to `main` with `--no-ff`, push, or leave the branch standing. If merged,
   verify the merged tree against the tested tip by tree hash (the root checkout has no
   `node_modules`), then the staging redeploy is a separate decision again. **This phase adds no
   migration** (26 stays), so its own redeploy is a code-only `scripts/deploy-staging.ps1 -Force`
   (`-Force` because the merge would happen on this laptop) — **but Phase 27's migration 26
   `case_insensitive_names` is still pending on staging and applies on that same redeploy, so run
   the seven-statement duplicate check in §1's Database row first** (item 2 below has the
   full wording). Verify afterwards exactly as Phase 26 did: `docker compose ps` (web healthy),
