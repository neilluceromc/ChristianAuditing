...
| Last two phases | **Phase 22** — Stock control D2: FIFO lot consumption and costing, optional expiry, three report pages with exports, and receipt documents, closing out the six D1 leftovers (spec `2026-09-16-stock-control-d2-design.md` §0). Every inflow is a lot and every outflow is split across lots by a new append-only `StockAllocation` table, so a lot's remaining quantity and every value are derived, never stored (decision 2); uncosted lots (opening stock, positive adjustments) are real but valueless — an issue drawing from one shows cost as "—", and a lot's null cost can be set once (decision 3); expiry is optional on any lot, with earliest-expiring-first consumption on issue and write-off, undated lots last, expired lots skipped by issues and taken first by adjustments (decision 4). Three report pages, each with an xlsx export: on hand and value (category subtotals, an uncosted toggle), consumption by department and month (grid, flat export), and expiring/expired lots (window chips, wired to the real write-off dialog) (decision 5). `StockLotDocument` attaches receipt documents to a lot, mirroring `RequestDocument` (decision 6). Purchasing's Home gains two tiles, "Below reorder level" and "Expiring within 30 days" (decision 7). All six D1 leftovers close here (decision 8, spec §8): the importer refuses an archived category and closes its two-key duplicate check; `SupplierOption.archived`, the redundant `state === "OPEN"` re-check, and the always-true `canCount` prop are all removed; the import's unit cost is honoured on CREATE rows. Migration 23's plpgsql backfill gives D1's history a lot per lot-less inflow and allocations for every outflow, asserting the result before it commits (decision 9) — verified 0 failures over 25 movements / 19 lots on `inventory_dev`. One additive migration, **23** (`stock_lots_and_allocations`). Spec `superpowers/specs/2026-09-16-stock-control-d2-design.md`, plan `superpowers/plans/2026-09-16-phase-22-stock-control-d2.md` (10 tasks, `D-1`…`D-20` — the final-review fix wave added `D-16`…`D-19`, the final battery `D-20`): ruling R6 had the movement actions and stocktake posting revalidate the three report pages, which no task's file list had named; ruling R7 fixed the consumption report's UTC-midnight range bounding (`manilaDayBounds`, `to` inclusive) so a transaction after 08:00 Manila on the report's last day was no longer dropped, and added the on-hand toggle's missing `uncosted` facet; ruling R9 had the reports adopt the Stock list's own multi-select `FacetDropdown` for Category instead of a native `<select>`, and share one `EXPIRED`/`EXPIRING` pill component across the item page, the list and the reports. The final whole-branch review then found two Important findings — the expiry report counted archived items' lots while the Home tile and the on-hand report both already excluded them, and the importer's opening-stock inflow never revalidated the three report pages like the other two writers already did — plus four parked Minors, all fixed in one wave (`2eeabf7`); the battery re-run that followed caught one pre-existing e2e flake on its own, a hydration-race gap in `it-core.spec.ts`'s documents-upload case, fixed test-only (`a4b57b5`). Battery in the table above. **CODE-COMPLETE on `phase-22-stock-control-d2` (final tree `a4b57b5`, 2026-09-16, after the final-review fix wave); UNMERGED and UNPUSHED** — merging, pushing and the staging redeploy are the user's decisions; `docs/HANDOVER-PENDING.md` §5.1 is shipped, its requisition question decided no (2026-09-16). **Phase 21** — Approvals oversight: direct IT changes become visible and distinguishable on the approvals surfaces, without reopening Phase 15's "applies now, no pending row" decision (spec §0 decision 1). `Approval.appliedDirectly` (migration 22, additive, backfilled once at creation) marks a direct change; the Closed tab shows it as a `DIRECT` pill and filters by route (`via=direct` / `via=queue`); a direct row's detail page says who applied it and when instead of a checked-request summary. Admins get a Home section, "Applied directly · last 7 days", with counts by kind and by person, on both Homes an admin can land on (decision 4). One approval now appears once per Home screen — the worklist no longer repeats the viewer's own claims already shown in "Claimed by you" — settling the "possible approvals double-count" Phase 17's final review left undecided (decision 5). Every paged list now reads its count and its page from one `pagedSnapshot` transaction, closing Phase 17's deferred "count then fetch can shift" note for all lists at once (decision 6). Every reason field strips invisible characters before its length check, through one shared schema (decision 7). The axe sweep's four tolerated rules (`empty-table-header`, `page-has-heading-one`, `heading-order`, `landmark-unique`) are attributed to their root causes, fixed, and promoted to failing rules (decision 8). The rate limiter gets its first end-to-end coverage, one mutation cap and one import cap, each proving the real `RateLimitNotice` render (decision 9). One additive migration, **22**. Spec `superpowers/specs/2026-09-14-approvals-oversight-design.md`, plan `superpowers/plans/2026-09-14-phase-21-approvals-oversight.md` (8 tasks, `D-1`…`D-16` — the final-review fix wave added `D-11`…`D-13`, the final battery `D-15` and `D-16`): ruling R5 fixed the worklist's own-claims exclusion, written as `NOT: { claimedById: userId }`, which SQL's `<>` never matches against a NULL `claimedById` — every unclaimed breach had vanished from Home's worklist until the clause spelled out the null case explicitly. The final whole-branch review then found two Important findings — `cleanReason` was deleting tabs and newlines from every multi-line reason, and `pagedSnapshot` inherited Prisma's 2 s/5 s transaction defaults on eighteen lists — both fixed in one wave (`b6b4e2a`); the battery re-run that followed caught a third, real bug on its own: at 01:00 Manila the Transfer dialog and schema computed "today" in UTC instead of Asia/Manila, a same-day transfer parked as Phase 20's M-5, fixed with a shared `localDateISO` helper (`2ae8738`). Battery in the table above. **MERGED TO `main` via `--no-ff` `b716767` AND PUSHED (2026-09-15) from final tree `2ae8738`; the merged tree is byte-identical to the tested tip (tree hashes matched); branch and worktree removed; staging redeployed the same day with `-Force` (migration 22 applied, web healthy).** — merging, pushing and the staging redeploy are the user's decisions; `docs/HANDOVER-PENDING.md` holds the parked work. |
...
1. **Phase 22 — Stock control D2: FIFO lot consumption and costing, optional expiry, three report pages
   with exports, receipt documents, and the six D1 leftovers — IS CODE-COMPLETE on
   `phase-22-stock-control-d2` at final tree `a4b57b5` (2026-09-16, after the final-review fix wave);
   branch and worktree still present** (10 tasks, `D-1`…`D-20` — the final-review fix wave added
   `D-16`…`D-19`, the final battery `D-20`) —
   **UNMERGED and UNPUSHED: merging to `main`, pushing, and the staging redeploy (migration 23 applies on
   start) are the user's decisions, not pre-authorised.** See §1's table above and `HANDOVER.md` (o) for
   what shipped and the battery; `docs/HANDOVER-PENDING.md` §5.1 is shipped, its requisition question
   decided no (2026-09-16).
2. **Phase 21 — Approvals oversight: direct IT changes made visible, plus five correctness and quality
   ...
   the battery.
3. **Phase 20 — IT side: transfers, offboarding attribution and facets, directory quality, six known
...
4. **Phase 19 — Stock control, part D1 (the ledger core) — is DONE, including the final-review fix
   ...
   **merged to `main` via `--no-ff` `d311e26` and pushed (2026-09-10); branch and worktree removed; staging
   ...
   is next (item 8 below); Purchasing's opening stock goes in through `/stock/import`.**
5. **Phase 18 — Purchasing: suppliers and request extensions — is DONE, including the
   ...
   and what it changed).
6. **Deploy the prototype to the staging laptop and let Purchasing and Finance use it.** Chosen by the user
   ...
   scan one QR).
7. **Two visual checks never done by a human:** the inventory class-switch chips and Finance's IT/Purchasing
   tabs by eye, and the Register form signed in as `purchasing@`. Both are asserted by e2e and axe, never
   looked at.
8. **Remaining candidates**, in value order (**Phase 16 delivered the fourth item this list used to
...
9. **The §9 subsystems from the Admin meeting — B and C shipped in Phase 18; D's first half (D1) shipped
   in Phase 19 (item 4 above); D2 shipped in Phase 22 (this branch, item 1 above).** Vendor master data (B)
   ...
   **D1 · stock control** (items, the ledger, derived balances, stocktake, import) is merged to `main`
   (`d311e26`, 2026-09-10; staging still to be redeployed). **D2 · FIFO costing, expiry warnings, report
   views and exports by department and month** shipped in Phase 22 as stock control D2 (this branch) —
   CODE-COMPLETE at final tree `a4b57b5`, UNMERGED and UNPUSHED (`HANDOVER.md` §9, item 1 above).
