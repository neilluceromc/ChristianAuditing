...
| Database | **22 migrations** on `main` (merge `b716767`; migration 22 `approval_applied_directly`, additive, one-statement backfill; 21 `employee_transfers`, 20 `stock_control`); staging runs **21** (the 2026-09-10 `-Force` redeploy applied 20 and 21) — migration 22 applies at the next `scripts/deploy-staging.ps1 -Force`. `prisma migrate reset` is **not** used in this project. |
...
1. **Phase 21 — Approvals oversight: direct IT changes made visible, plus five correctness and quality
   ...
   battery `D-15` and `D-16`; final tree `2ae8738`) — **the staging redeploy (migration 22, one
   `scripts/deploy-staging.ps1 -Force`) is the user's decision, not pre-authorised.** See §1's table above and `HANDOVER.md` (n) for what shipped and
   the battery.
