...
1. **Phase 31 — the leftovers sweep (a derived sort orders only as the primary key, both exports
   ...
   that are not links) IS CODE-COMPLETE on `phase-31-leftovers-sweep` at final tree `7599d9e`, UNMERGED
   and UNPUSHED; no migration.** Bounded, no spec or plan; design, the seven rulings and the battery
   (413 e2e / 40 files, 1634 unit / 96 files) in `HANDOVER.md` (x). **The finishing decision is the
   user's:** merge `--no-ff` into `main`, compare the tip's tree with the merge commit's tree, push
   only `main`; the staging redeploy is a separate, code-only decision.
