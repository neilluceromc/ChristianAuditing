...
| Database | 19 migrations, additive only, on `main` (staging is still at 18 until the next `-Force` redeploy). `prisma migrate reset` is **not** used in this project. |
...
1. **Phase 18 — Purchasing: suppliers and request extensions — is DONE, including the
   ...
   worktree removed; staging not yet redeployed**: a supplier master record (profile, contract, encrypted bank accounts, documents, search)
   ...
   remains: redeploy staging with `scripts/deploy-staging.ps1 -Force` (migration 19 applies on start);
   Purchasing's cleaned supplier list goes in through `/purchases/suppliers/import`; item D
