...
1. **Phase 28 — a Back control on every page that has a parent: where the operator came
   ...
   adapts) and what stays parked. **Staging is NOT yet redeployed — that redeploy is the user's
   decision, not pre-authorised.** **This phase adds no
   migration** (26 stays), so its own redeploy is a code-only `scripts/deploy-staging.ps1 -Force`
   (`-Force` because the merge happened on this laptop) — **but Phase 27's migration 26
