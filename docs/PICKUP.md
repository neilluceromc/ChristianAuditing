...
| Battery, last run 2026-09-16 (Phase 22 final tree `dfb1a0f`, `phase-22-stock-control-d2`, unmerged, before the docs commit) | `tsc` clean · `lint` clean · **23 migrations**, schema up to date · **1404 unit / 79 files** · **319 e2e / 31 files** by `--list`. Task 10's close ran seven foreground chunks, `E2E_PORT=3100 --workers=1 --global-timeout=540000` (chunk E split into E1/E2 per spec §9.3, since the combined chunk would have carried 67 tests): **A 54 passed (3.4m) · B 67 passed (3.9m) · C 63 passed (4.1m) · D 38 passed (2.7m) · E1 36 passed (5.6m) · E2 31 passed (3.0m) · F 30 passed (3.2m) = 319 / 319**, zero failed, zero did-not-run, all first pass, no re-run needed anywhere in the battery. `npm run db:seed` ran last, clean; port 3100 carried no listening process before the first chunk, between any two chunks, or after the last. Commands and history: `HANDOVER.md` §0 item 9; what shipped: `HANDOVER.md` (o). |
...
1. **Phase 22 — Stock control D2: FIFO lot consumption and costing, optional expiry, three report pages
   ...
   `phase-22-stock-control-d2` at final tree `dfb1a0f` (2026-09-16); branch and worktree still present**
   (10 tasks, `D-1`…`D-15` — `D-16`+ reserved for the final whole-branch review and its fix wave) —
...
9. **The §9 subsystems from the Admin meeting — B and C shipped in Phase 18; D's first half (D1) shipped
   in Phase 19 (item 4 above); only D2 remains.** Vendor master data (B) and purchasing workflow
