...
1. **Phase 29 — Laws of UX applied to the employee area (the profile's state-chosen primary with
   Edit kept and a More menu, slot tiles that open an action menu, the New form's live checks with
   every error in one pass, the list as a worklist with a sortable Loadout column) IS CODE-COMPLETE
   on `phase-29-employee-uiux` at final tree `9660e6e`, UNMERGED and UNPUSHED.** Spec
   ...
   table (389 e2e / 38 files, 1556 unit / 90 files, 26 migrations). **The finishing decision is the
   user's, not pre-authorised:** merge `--no-ff` into `main`, then compare
   `git rev-parse phase-29-employee-uiux^{tree}` with the merge commit's tree — the root checkout
   has no `node_modules`, so the merged tree cannot be re-tested there and the tree-hash check is
   what stands in for it. Then push **only `main`**, which also carries the two unpushed docs-only
   commits `401b838` and `1acd928`. **The staging redeploy is a SEPARATE decision.** Phase 29 adds
   **no migration** (26 stays), so its own redeploy is code-only — but the first
   `scripts/deploy-staging.ps1 -Force` after the merge still applies Phase 27's pending
   **migration 26** `case_insensitive_names`, so run the seven-statement duplicate check in §1's
   Database row first (the full wording is in the Phase 27 item below). `inventory_dev` is clean:
