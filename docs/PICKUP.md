...
1. **Phase 18 — Purchasing: suppliers and request extensions — is CODE-COMPLETE**, on
   `phase-18-purchasing-suppliers`, **UNMERGED and UNPUSHED at the time of writing**: a supplier master
   record (profile, contract, encrypted bank accounts, documents, search) under a new Suppliers area at
   `/purchases/suppliers`; a required department and an optional supplier on every purchase request, with
   list filters/columns and role-gated attachments; an asset's provenance (from a purchase request,
   registered directly, or a historical import) derived and shown everywhere — the inventory facet, the
   detail badge, the export and Finance columns. One additive migration, **19**. Spec
   `superpowers/specs/2026-09-08-purchasing-suppliers-design.md`, plan
   `superpowers/plans/2026-09-08-phase-18-purchasing-suppliers.md` (17 amendments, `D-1`…`D-17`). Battery:
   `tsc`/`lint` clean, 1110 unit / 64 files, 258 e2e / 21 files across four foreground chunks (54 · 67 ·
   63 · 74 = 258, zero failed, zero did-not-run) — see the table in §1 above and `HANDOVER.md` (k). **What
   remains: the merge/push and staging redeploy are the user's decisions (not pre-authorised for this
   phase); Purchasing's cleaned supplier list goes in through `/purchases/suppliers/import`; item D
