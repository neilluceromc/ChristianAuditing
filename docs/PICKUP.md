...
1. **Phase 25 — IT navigation sweep: start offboarding from the profile, every reference a link, the
   ...
   `scripts/deploy-staging.ps1 -Force` (`-Force` because the merge happened on this laptop) — **NOT yet run**:
   staging still serves the Phase 24 merge `48c37f7`. The push also carried the three docs-only commits `9dd7ba5`,
