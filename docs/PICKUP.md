...
| Last two phases | **Phase 26** — holds that work, and the repair end-date. The two lines Phase 25's audit split off because both needed schema work. A hold can now be **placed** — from the asset record's **Reserve** control or from an empty policy slot's **Reserve a spare…** menu item on a person's profile, with **For**, an **Expires** date prefilled to today + 7 and floored at today, and a reason — **released** from the record, the profile's holding area or `/reservations`, and **expired** by the worker, which flips ACTIVE holds past their Manila day to EXPIRED once at start and hourly after and logs `[worker] expired N hold(s)`; expiry writes no audit row (decision 7), a placement and a release write exactly one each (`reservation.placed` / `reservation.released`). The expiry is visible everywhere the hold is — the inventory HOLD cell, the record's held banner and Reservations tab, the profile's holding area and the list — and `/reservations` itself was rebuilt to list parity: search over tag, model and person, Employee and Department facets, four sortable headers, whole-row click, Release for admin/IT and pagination that keeps tab, search and facets. `Asset.repairEndedAt` (**migration 25**, additive and nullable, with a backfill that fills only rows whose audit history shows a transition off DEFECTIVE) is stamped by `prepareLifecycle` on every path that leaves DEFECTIVE and cleared when an asset re-enters it, so `downDays` closes its interval and a returned-OK asset reads “down N d, back since {date}” — or “closed {date}” once the status family is closed — instead of a dash. `canAssign` on the record is byte-for-byte unchanged. Spec `superpowers/specs/2026-09-21-holds-and-repair-end-design.md`, plan `superpowers/plans/2026-09-21-phase-26-holds-and-repair-end.md` (6 tasks, `D-1`…`D-10` — the spec corrections before the plan are `D-2`, the final whole-branch review added `D-8`, the fix wave `D-9`, the final battery `D-10`), five rulings: R1 recorded the branch point from local `main`; R2 accepted Task 1's two type-only lines in `e2e/it-gaps.spec.ts` so `tsc` stayed green; R3 fixed the Assign dialog preselecting a holder who might no longer be assignable; R4 set the fix wave's contents; R5 closed the last wall-clock `expiresAt` writer (the paging scale fixture) inside the wave. The final whole-branch review found 0 Critical, 3 Important, 9 Minor: `reserveAsset` had no `P2002` branch although the partial unique index `Reservation_one_active_hold_per_asset` has existed since migration `20260814090100` — spec §0 decision 8's premise was wrong — so a lost race threw past the action instead of answering with the pinned conflict copy; the `/reservations` tab badges counted unfiltered while the tab links carried the filters; and `holdStatus` compared Manila calendar days while the sweep compared raw instants, equal only for UTC-midnight rows, which the seed did not write. All fixed in one wave of two commits (`bbaec18`, then `1442289` for ruling R5), with day-precision `expiresAt` now declared in `schema.prisma` and honoured by every writer; the scoped re-review came back CLEAN. Battery in the table above. **CODE-COMPLETE and UNMERGED at final tree `1442289`; merging, pushing and the `-Force` staging redeploy that applies migration 25 are the user's decisions.** Recorded rather than fixed: `holdStatus` also says “expires tomorrow” at one day (mirroring `dueStatus`), and `holdFacets` lists every employee with a hold in the tab without a `take` (correct at team scale). Declined: the tabs' import-plus-re-export shape, and the Employee facet asymmetry — fixed instead by giving the Department facet the same dimmed zero counts. **Phase 25** — IT navigation sweep: start offboarding from the profile, every reference a link, the new-hire finish line, employees list parity, safety nets. An audit-first sweep: the whole IT surface (47 routes) was mapped for dead ends and unwired affordances, ranked by clicks saved on the four daily IT paths (new hire, swap/repair, move/reassign, leaver), and what needed schema work was split off into Phase 26 (spec §0 decisions 1 and 2). A new `startOffboarding` server action — a faithful twin of the Edit-form path, same guard order, same floor, one transaction with one audit row, the same seven revalidations — puts a **Start offboarding** button with a **Complete by** date (five working days by default) on the profile header, so a leaver no longer starts through the Edit form; when the employee is frozen the banner carries the date as a `DuePill`, and the wizard header's pill reads `closed` once they are OFFBOARDED. Eight reference sites became links — the asset record's pending banner, the asset timeline's ref, the employee timeline's refs and tags, both IT activity feeds (a trailing entity chip), the `HOLD` pill's holder, the profile's five newest open requests, the Home fleet bar's segments and legend (`/inventory?status=…`), and `department` in the audit page's entity labels. Creating an employee now lands on their profile with a banner offering **Assign devices** and **Accountability form**, the Name field takes focus on the form, and `/employees` finally renders the sort headers its config has always honoured, with focusable rows that open on click or Enter. The safety nets: an `(app)/error.tsx` inside the shell, five `loading.tsx` skeletons, the wizard's `not-found.tsx`, offboarding facets that narrow each other, and an offboarding Export mirroring `/employees/export`. **No migration, no new table, no role change.** Spec `superpowers/specs/2026-09-21-it-navigation-sweep-design.md`, plan `superpowers/plans/2026-09-21-phase-25-it-navigation-sweep.md` (8 tasks, `D-1`…`D-12` — the final whole-branch review added `D-10`, the fix wave `D-11`, the final battery `D-12`), four rulings: R1 recorded that the worktree branched from local `main` so the spec and plan commits sit in the branch's history, as in Phases 20–24; R2 kept the fleet bar's real links and had Task 7 rewrite the `role="img"` assertion in `e2e/home-finance.spec.ts` rather than weaken the markup; R3 established that `AuditEntry` is append-only at the database, so no e2e case deletes audit rows — they restore mutable fields and rely on the file's `beforeAll` reseed; R4 fixed the wave's contents. The final whole-branch review found 0 Critical, 5 Important, 8 Minor, none of them a correctness or data defect: three user-visible polish defects a single-page walk could not catch (a record-level skeleton drawing a second header and tab strip *under* the real ones, one `loading.tsx` covering the wizard and the farewell report as well as the list, and segment links whose focus ring was clipped by an `overflow-hidden` wrapper), one comment that had become false (`updateEmployee` is no longer the only writer of `offboardingDueAt`), and two e2e assertions sitting below the spec's own §8 list — case 1 asserted the audit row's count but never its three diff fields, which is exactly the check that makes the parked hand-built-diff Minor safe to park. All fixed in one wave (`fab0780`, seven files); the scoped re-review found all nine ADDRESSED plus the folded-in Minor, 0 new. Battery in the table above. **MERGED TO `main` via `--no-ff` `bb577f8` AND PUSHED 2026-09-21 (tree hashes matched the tested tip `641e96f`); branch and worktree removed; staging redeployed 2026-09-21 with a code-only `-Force` and serves `5727572`.** Recorded rather than fixed: the HOLD holder link dead-ends for `finance_staff` (the house's existing assignee-link behaviour); `autoFocus` is redundant with the focus trap; a focusable `<tr>` carries no role (spec §5.2's choice); `startOffboarding` writes `offboardingAt = now` where the Edit path preserves it (unreachable); and the frozen banner shows the due pill for OFFBOARDING only, so a closed case can never read "overdue". Phase 24's entry rolled off this row at Phase 26's close, as Phase 23's did at Phase 25's and Phase 22's at Phase 24's — both are unchanged in §4 below and in `HANDOVER.md` (q) and (p). |
...
1. **Phase 26 — holds that work + a repair end-date: reserve a spare from the asset record or from
   an empty profile slot, release it, an hourly expiry sweep, `/reservations` list parity, and
   `Asset.repairEndedAt` stamped by the lifecycle preparer — IS CODE-COMPLETE on
   `phase-26-holds-and-repair-end` at final tree `1442289` (after the final-review fix wave), UNMERGED
   and UNPUSHED** (6 tasks, `D-1`…`D-10` — the spec corrections before the plan are `D-2`, the
   final whole-branch review added `D-8`, the fix wave `D-9`, the final battery `D-10`). **Merging,
   pushing and the staging redeploy are the user's decisions, not pre-authorised.** The finishing menu
   is `superpowers:finishing-a-development-branch`: merge into `main` with `--no-ff`, compare the
   merged tree hash against the tested tip, push, then redeploy staging with
   `scripts/deploy-staging.ps1 -Force` (`-Force` because the merge happens on this laptop) — and
   **this redeploy is not code-only: migration 25 `repair_end_and_holds` applies on it** (additive, one
   nullable column; its backfill fills `repairEndedAt` only where an audit row moved a status off
   DEFECTIVE, so nothing is invented and `BR-MN-0911`-style rows keep their dash). Local `main` also
   carries the two unpushed docs-only commits `2bed93b` (spec) and `611d91f` (plan and spec
   corrections) beyond `origin/main` `6530623`. See §1's table above and `HANDOVER.md` (s) for what
   shipped, the five rulings, the final review and the battery; what stays parked is in
   `docs/HANDOVER-PENDING.md` §6 and §5 below.
2. **Phase 25 — IT navigation sweep: start offboarding from the profile, every reference a link, the
...
3. **Phase 24 — IT gaps 2: Replace groups, the repairs view paged in SQL, worker retention — IS
...
4. **Phase 23 — Parkinson sweep: quick forms and complete-by dates — IS MERGED TO `main` via
   ...
   **Staging redeployed 2026-09-17 with `scripts/deploy-staging.ps1 -Force`: migration 24 applied (24 found), web healthy, the LAN URL answers; the one leaver already on staging now shows a complete-by date and reads overdue, as intended. Nothing remains for this phase.**
   ...
   plan `D-15`.
5. **Phase 22 — Stock control D2: FIFO lot consumption and costing, optional expiry, three report pages
...
6. **Phase 21 — Approvals oversight: direct IT changes made visible, plus five correctness and quality
   ...
   the battery.
7. **Phase 20 — IT side: transfers, offboarding attribution and facets, directory quality, six known
...
8. **Phase 19 — Stock control, part D1 (the ledger core) — is DONE, including the final-review fix
...
9. **Phase 18 — Purchasing: suppliers and request extensions — is DONE, including the
   ...
   and what it changed).
10. **Deploy the prototype to the staging laptop and let Purchasing and Finance use it.** Chosen by the user
   ...
   scan one QR).
11. **Two visual checks never done by a human:** the inventory class-switch chips and Finance's IT/Purchasing
   tabs by eye, and the Register form signed in as `purchasing@`. Both are asserted by e2e and axe, never
   looked at.
12. **Remaining candidates**, in value order (**Phase 16 delivered the fourth item this list used to
...
13. **The §9 subsystems from the Admin meeting — B and C shipped in Phase 18; D's first half (D1) shipped
...
- **Closed in Phase 26 — holds that work, and a repair end-date.** Phase 25's audit turned two
  ...
  `phase-26-holds-and-repair-end` delivered both (final tree `1442289`; unmerged and unpushed).
