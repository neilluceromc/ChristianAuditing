...
1. **Phase 21 — Approvals oversight: direct IT changes made visible, plus five correctness and quality
   items — IS CODE-COMPLETE on `phase-21-approvals-oversight`, UNMERGED and UNPUSHED; branch and worktree
   still present** (8 tasks, `D-1`…`D-16` — the final-review fix wave added `D-11`…`D-13`, the final
   battery `D-15` and `D-16`; final tree `2ae8738`) — **merging to `main`, pushing, and the
   eventual staging redeploy (migration 22 would ride the same `-Force` redeploy as 20 and 21) are all the
   user's decisions, not pre-authorised.** See §1's table above and `HANDOVER.md` (n) for what shipped and
   the battery.
2. **Phase 20 — IT side: transfers, offboarding attribution and facets, directory quality, six known
...
3. **Phase 19 — Stock control, part D1 (the ledger core) — is DONE, including the final-review fix
   ...
   **merged to `main` via `--no-ff` `d311e26` and pushed (2026-09-10); branch and worktree removed; staging
   ...
   is next (item 7 below); Purchasing's opening stock goes in through `/stock/import`.**
4. **Phase 18 — Purchasing: suppliers and request extensions — is DONE, including the
   ...
   and what it changed).
5. **Deploy the prototype to the staging laptop and let Purchasing and Finance use it.** Chosen by the user
   ...
   scan one QR).
6. **Two visual checks never done by a human:** the inventory class-switch chips and Finance's IT/Purchasing
   tabs by eye, and the Register form signed in as `purchasing@`. Both are asserted by e2e and axe, never
   looked at.
7. **Remaining candidates**, in value order (**Phase 16 delivered the fourth item this list used to
...
8. **The §9 subsystems from the Admin meeting — B and C shipped in Phase 18; D's first half (D1) shipped
   in Phase 19 (item 3 above); only D2 remains.** Vendor master data (B) and purchasing workflow
