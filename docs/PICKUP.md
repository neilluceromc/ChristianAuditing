...
1. **Phase 27 — IT and quality leftovers sweep: `/audit` class scoping of approval, category and
   ...
   the final battery `D-10`). **Staging is NOT yet redeployed — that redeploy is the user's decision,
   not pre-authorised.** When asked, run `scripts/deploy-staging.ps1 -Force` (`-Force` because the
   merge happened on this laptop). This phase
...
- **Closed in Phase 27 — the three IT items every phase since 24 carried forward.** `/audit` class
  ...
  `phase-27-leftovers-sweep` (final tree `66445ed`, code-complete 2026-09-22; merged to `main` as `5dd3372` and pushed 2026-09-22; staging not yet redeployed). Two things the
