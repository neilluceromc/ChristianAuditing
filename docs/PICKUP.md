...
1. **Phase 20 — IT side: transfers, offboarding attribution and facets, directory quality, six known
   gaps — is CODE-COMPLETE, including the final-review fix wave, on `phase-20-it-people-and-gaps`,
   UNMERGED and UNPUSHED** (9 tasks, `D-1`…`D-30` — the final-review fix wave added `D-23`…`D-29`, the
   final battery `D-30`; final tree `4d54382`) — **merging, pushing and the staging redeploy are the user's
   decisions, not pre-authorised.** See §1's table and `HANDOVER.md` (m) for what shipped, the final
