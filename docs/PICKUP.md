...
1. **Phase 25 — IT navigation sweep: start offboarding from the profile, every reference a link, the
   new-hire finish line, employees list parity, safety nets — IS CODE-COMPLETE on
   `phase-25-it-navigation-sweep` at final tree `fab0780` (2026-09-21, after the final-review fix wave),
   but it is UNMERGED and UNPUSHED** (8 tasks, `D-1`…`D-12` — the final whole-branch review added
   `D-10`, the fix wave `D-11`, the final battery `D-12`). **This is the decision waiting for the user.**
   The finishing menu is the one Phases 20–24 used: review the branch, then merge to `main` with
   `--no-ff`, push, and redeploy staging — or keep it local. Nothing here is pre-authorised, and none of
   it has been done. There is **no migration** (24 stays), so the redeploy that carries it is a code-only
   `scripts/deploy-staging.ps1 -Force` (`-Force` is needed whenever the merge happened on this laptop,
   because the plain run sees nothing new and skips). Verify a merge by tree hash, as every phase since 20
   has: the branch's final tree is `fab0780`. Local `main` also carries three docs-only commits beyond
   `origin/main` `a9970ad`, themselves unpushed — `9dd7ba5` and `98a94a1` (the spec and its corrections)
   and `5acb5fd` (the plan) — plus this phase's own docs commit; a push carries all of them. See §1's
...
2. **Phase 24 — IT gaps 2: Replace groups, the repairs view paged in SQL, worker retention — IS
...
3. **Phase 23 — Parkinson sweep: quick forms and complete-by dates — IS MERGED TO `main` via
   ...
   **Staging redeployed 2026-09-17 with `scripts/deploy-staging.ps1 -Force`: migration 24 applied (24 found), web healthy, the LAN URL answers; the one leaver already on staging now shows a complete-by date and reads overdue, as intended. Nothing remains for this phase.**
   ...
   plan `D-15`.
4. **Phase 22 — Stock control D2: FIFO lot consumption and costing, optional expiry, three report pages
...
5. **Phase 21 — Approvals oversight: direct IT changes made visible, plus five correctness and quality
   ...
   the battery.
6. **Phase 20 — IT side: transfers, offboarding attribution and facets, directory quality, six known
...
7. **Phase 19 — Stock control, part D1 (the ledger core) — is DONE, including the final-review fix
...
8. **Phase 18 — Purchasing: suppliers and request extensions — is DONE, including the
   ...
   and what it changed).
9. **Deploy the prototype to the staging laptop and let Purchasing and Finance use it.** Chosen by the user
   ...
   scan one QR).
10. **Two visual checks never done by a human:** the inventory class-switch chips and Finance's IT/Purchasing
   tabs by eye, and the Register form signed in as `purchasing@`. Both are asserted by e2e and axe, never
   looked at.
11. **Remaining candidates**, in value order (**Phase 16 delivered the fourth item this list used to
...
12. **The §9 subsystems from the Admin meeting — B and C shipped in Phase 18; D's first half (D1) shipped
...
- **Parked for Phase 26 — reservations that work, and a repair end-date.** Phase 25's audit turned
  two long-standing lines in `HANDOVER.md` §8 into a named next phase. **Reservations:** nothing in
  `src/server` ever creates a `Reservation` — the only writer anywhere is `prisma/seed.ts` —
  `HolderControl` exposes only `assign` and `return`, and although `ReservationState` carries `RELEASED`
  and `EXPIRED`, the single state write in the codebase is `ACTIVE → FULFILLED` when the reserved
  employee is assigned the asset. So `/reservations`' own empty-state copy ("Reserve a spare from an asset
  record…") and its banner ("Holds are placed and released on the asset record") both promise a control
  that does not exist, every seeded hold is permanent until fulfilled, and nothing sweeps an expired hold,
  so a long-expired one still renders as live. **Repair end-date:** `RETURNED OK` shows `—` in the Down
  column because `downDays` returns `null` for anything not `status === "DEFECTIVE"`, and nothing anywhere
  records when a repair actually ended, so a completed repair's downtime cannot be reported. Both need
  schema work, which is why Phase 25 (no migration) deliberately left them; Phase 26 gets its own
  brainstorm. Full facts: `docs/HANDOVER-PENDING.md` §6.
