...
1. **Phase 24 — IT gaps 2: Replace groups, the repairs view paged in SQL, worker retention — IS
   CODE-COMPLETE on `phase-24-it-gaps-2` at final tree `7601cab` (after the final-review fix wave), and
   is UNMERGED and UNPUSHED; the branch and its worktree `.claude/worktrees/phase-24-it-gaps-2` are
   still in place** (6 tasks, `D-1`…`D-10` — the final whole-branch review added `D-8`, the fix wave
   `D-9`, the final battery `D-10`) — **the finishing menu is the user's to pick and none of it is
   pre-authorised: merge to `main` with `--no-ff` and push, keep the branch as it is for now, or
   discard it.** There is **no migration** (24 stays), so whenever the merge does happen the staging
   redeploy is a code-only `scripts/deploy-staging.ps1 -Force` and staging's schema does not move. Local
   `main` also carries the two docs-only commits `0aeb8ce` (spec) and `32e6959` (plan) beyond
   `origin/main` `feb93f3`, themselves unpushed — a push would carry them along the way Phases 21–23'
   pushes carried theirs. See §1's table above and `HANDOVER.md` (q) for what shipped, the four rulings,
