...
| Last two phases | **Phase 27** — IT and quality leftovers sweep. The three IT items every phase since 24 carried forward, closed together, plus the parked Minors of Phases 17–26 that were cheap or user-visible. **`/audit` is class-scoped properly:** `invisibleAuditRefs(role)` builds four id lists from the SEE map — assets, approvals, asset categories and asset types — so an IT or viewer account no longer reads Purchasing approvals, categories or types in the log or the export, a class-less approval is never hidden, and an all-class role runs no query at all; the Entity facet gained Supplier, Stock item, Stock category and Stocktake, and `entityLabels` resolves category and type rows to names instead of truncated cuids. **Reference names are unique whatever the case:** **migration 26** `case_insensitive_names` adds seven `lower()` UNIQUE indexes (asset categories, types scoped by category, departments, equipment policies, vendors, stock categories by name and by prefix) and every create and rename checks case-insensitively inside its own transaction first, naming the row it clashes with — `That name is already taken by “…”`, the policy, supplier and stock variants, and `That prefix is already in use by “…”`; renames exclude their own row, so “Laptop” → “LAPTOP” is still a rename. The five hand-rolled `P2002` helpers became one shared `isUniqueViolation`/`uniqueTarget` pair in `src/server/prisma-errors.ts`. **The four activity feeds became usable:** one shared module (`src/lib/activity-list.ts`) and one server component (`ActivityListPage`, behind four five-line pages) give inventory, employees, finance and purchases an **Action** facet with counts from the same snapshot as the rows, a Clear link, a live entry count and a filtered empty state; every action that can reach a feed has a plain-English sentence and a facet label, and `update`/`import-update` print human field names sorted on the label (“updated department, join date on Nina Robles by import”) instead of raw column keys. **The visible smalls:** `EntityCombobox` no longer opens on focus (so `/stock/issue` and `/stock/receive` stop rendering a second listbox on mount), the receive form leads with “No supplier”, Purchasing Home's “Stocktakes past close-by” tile turns accent when overdue, the work page's capped note counts the rows it renders, and `/inventory` rows open on Enter through the shared `rowOpenProps`. Spec `superpowers/specs/2026-09-22-leftovers-sweep-design.md`, plan `superpowers/plans/2026-09-22-phase-27-leftovers-sweep.md` (7 tasks, `D-1`…`D-10` — the pre-flight rulings R1–R4 are `D-1`, the final whole-branch review added `D-8`, the fix wave `D-9`, the final battery `D-10`), six rulings: R1 recorded the branch point from local `main`; R2 let Task 1 end with exactly three `tsc` errors in the audit path for Task 2 to clear; R3 made the planned parallel pair sequential; R4 corrected the plan's `parseIntParam(…0)` expectation; R5 scoped the `P2002` consolidation to the five modules the spec names and recorded six inline checks elsewhere; R6 set the fix wave's contents. The final whole-branch review found 0 Critical, 2 Important, 6 Minor: the approvals worker writes five `<lifecycle.*> executed` asset actions that reached the feeds with no label and no sentence, so the new facet listed a raw string beside “Assigned” and choosing “Assigned” dropped every queue-executed assign; and `entityLabels` printed truncated cuids for exactly the two entity types this phase class-scopes. Both fixed in one wave (`66445ed`), along with sorting field lists on the label, dropping a stray `?q=` from the feeds' hrefs, it-gaps case 3's fixtures, the clear option's id and the `document.signed` single-key contract; the scoped re-review came back CLEAN. Battery in the table above. **CODE-COMPLETE on the branch at `66445ed`, UNMERGED and UNPUSHED; migration 26 is pending on staging until the merge and the first `-Force` after it — run the duplicate check in the Database row first.** Recorded rather than fixed: `/audit`'s entity pill still renders the raw `vendor` while the facet and chip say “Supplier” (M-2 — the pill is CSS-uppercased and an existing e2e asserts `ASSET` on it, so it needs its own e2e pass), and six inline `P2002` checks outside the five consolidated modules (R5). Three decisions were recorded rather than built: no `role="group"` combobox rewrite, the Home age histogram stays unlinked, and keyboard support stays at Enter on a focused row. **Phase 26** — holds that work, and the repair end-date. The two lines Phase 25's audit split off because both needed schema work. A hold can now be **placed** — from the asset record's **Reserve** control or from an empty policy slot's **Reserve a spare…** menu item on a person's profile, with **For**, an **Expires** date prefilled to today + 7 and floored at today, and a reason — **released** from the record, the profile's holding area or `/reservations`, and **expired** by the worker, which flips ACTIVE holds past their Manila day to EXPIRED once at start and hourly after and logs `[worker] expired N hold(s)`; expiry writes no audit row (decision 7), a placement and a release write exactly one each (`reservation.placed` / `reservation.released`). The expiry is visible everywhere the hold is — the inventory HOLD cell, the record's held banner and Reservations tab, the profile's holding area and the list — and `/reservations` itself was rebuilt to list parity: search over tag, model and person, Employee and Department facets, four sortable headers, whole-row click, Release for admin/IT and pagination that keeps tab, search and facets. `Asset.repairEndedAt` (**migration 25**, additive and nullable, with a backfill that fills only rows whose audit history shows a transition off DEFECTIVE) is stamped by `prepareLifecycle` on every path that leaves DEFECTIVE and cleared when an asset re-enters it, so `downDays` closes its interval and a returned-OK asset reads “down N d, back since {date}” — or “closed {date}” once the status family is closed — instead of a dash. `canAssign` on the record is byte-for-byte unchanged. Spec `superpowers/specs/2026-09-21-holds-and-repair-end-design.md`, plan `superpowers/plans/2026-09-21-phase-26-holds-and-repair-end.md` (6 tasks, `D-1`…`D-10` — the spec corrections before the plan are `D-2`, the final whole-branch review added `D-8`, the fix wave `D-9`, the final battery `D-10`), five rulings: R1 recorded the branch point from local `main`; R2 accepted Task 1's two type-only lines in `e2e/it-gaps.spec.ts` so `tsc` stayed green; R3 fixed the Assign dialog preselecting a holder who might no longer be assignable; R4 set the fix wave's contents; R5 closed the last wall-clock `expiresAt` writer (the paging scale fixture) inside the wave. The final whole-branch review found 0 Critical, 3 Important, 9 Minor: `reserveAsset` had no `P2002` branch although the partial unique index `Reservation_one_active_hold_per_asset` has existed since migration `20260814090100` — spec §0 decision 8's premise was wrong — so a lost race threw past the action instead of answering with the pinned conflict copy; the `/reservations` tab badges counted unfiltered while the tab links carried the filters; and `holdStatus` compared Manila calendar days while the sweep compared raw instants, equal only for UTC-midnight rows, which the seed did not write. All fixed in one wave of two commits (`bbaec18`, then `1442289` for ruling R5), with day-precision `expiresAt` now declared in `schema.prisma` and honoured by every writer; the scoped re-review came back CLEAN. Battery in the table above. **MERGED to `main` via `--no-ff` `a5883ad` and PUSHED (2026-09-22); the merged tree is byte-identical to the tested tip `6175af3`; staging redeployed 2026-09-22 with `-Force`, migration 25 applied, web healthy.** Recorded rather than fixed: `holdStatus` also says “expires tomorrow” at one day (mirroring `dueStatus`), and `holdFacets` lists every employee with a hold in the tab without a `take` (correct at team scale). Declined: the tabs' import-plus-re-export shape, and the Employee facet asymmetry — fixed instead by giving the Department facet the same dimmed zero counts. Phase 25's entry rolled off this row at Phase 27's close, as Phase 24's did at Phase 26's and Phase 23's at Phase 25's — it is unchanged in §4 below and in `HANDOVER.md` (r). |
...
1. **Phase 27 — IT and quality leftovers sweep: `/audit` class scoping of approval, category and
   ...
   CODE-COMPLETE on `phase-27-leftovers-sweep` at final tree `66445ed` (after the final-review fix
   wave); UNMERGED and UNPUSHED; branch and worktree still in place** (7 tasks, `D-1`…`D-10` — the
   pre-flight rulings R1–R4 are `D-1`, the final whole-branch review added `D-8`, the fix wave `D-9`,
   the final battery `D-10`). **The finishing menu is the user's to pick from, and none of it is
   pre-authorised:** merge to `main` with `--no-ff`, push, then redeploy staging with
   `scripts/deploy-staging.ps1 -Force` (`-Force` because the merge happens on this laptop). This phase
   is **not** code-only — **migration 26** `case_insensitive_names` applies on that redeploy, and it
   FAILS LOUDLY on a database that already holds two names differing only by case, so **run the
   seven-statement duplicate check in §1's Database row first**; staging was clean on 2026-09-22 but it
   is a live database. Verify afterwards exactly as Phase 26 did: `docker compose ps` (web healthy),
   `docker compose exec -T web npx prisma migrate status` (**26 found, up to date**), HTTP 200 on
   `http://127.0.0.1:3000/login` and on the LAN URL. A push would also carry the two docs-only commits
   `385b484` (spec) and `8c24c3b` (plan) that have sat on local `main` since 2026-09-22. See §1's table
   ...
   below.
2. **Phase 26 — holds that work + a repair end-date: reserve a spare from the asset record or from
...
3. **Phase 25 — IT navigation sweep: start offboarding from the profile, every reference a link, the
...
4. **Phase 24 — IT gaps 2: Replace groups, the repairs view paged in SQL, worker retention — IS
...
5. **Phase 23 — Parkinson sweep: quick forms and complete-by dates — IS MERGED TO `main` via
   ...
   **Staging redeployed 2026-09-17 with `scripts/deploy-staging.ps1 -Force`: migration 24 applied (24 found), web healthy, the LAN URL answers; the one leaver already on staging now shows a complete-by date and reads overdue, as intended. Nothing remains for this phase.**
   ...
   plan `D-15`.
6. **Phase 22 — Stock control D2: FIFO lot consumption and costing, optional expiry, three report pages
...
7. **Phase 21 — Approvals oversight: direct IT changes made visible, plus five correctness and quality
   ...
   the battery.
8. **Phase 20 — IT side: transfers, offboarding attribution and facets, directory quality, six known
...
9. **Phase 19 — Stock control, part D1 (the ledger core) — is DONE, including the final-review fix
   ...
   **merged to `main` via `--no-ff` `d311e26` and pushed (2026-09-10); branch and worktree removed; staging
   ...
   shipped in Phase 22 (item 6 above); Purchasing's opening stock goes in through `/stock/import`.**
10. **Phase 18 — Purchasing: suppliers and request extensions — is DONE, including the
   ...
   and what it changed).
11. **Deploy the prototype to the staging laptop and let Purchasing and Finance use it.** Chosen by the user
   ...
   scan one QR).
12. **Two visual checks never done by a human:** the inventory class-switch chips and Finance's IT/Purchasing
   tabs by eye, and the Register form signed in as `purchasing@`. Both are asserted by e2e and axe, never
   looked at.
13. **Remaining candidates**, in value order (**Phase 16 delivered the fourth item this list used to
...
14. **The §9 subsystems from the Admin meeting — B and C shipped in Phase 18; D's first half (D1) shipped
   in Phase 19 (item 9 above); D2 shipped in Phase 22 (merged `26aaf82`, item 6 above).** Vendor master data (B)
   ...
   **D1 · stock control** (items, the ledger, derived balances, stocktake, import) is merged to `main`
   ...
   merged to `main` via `--no-ff` `26aaf82` and pushed 2026-09-16 (`HANDOVER.md` §9, item 6 above).
...
- **Closed in Phase 27 — the three IT items every phase since 24 carried forward.** `/audit` class
  ...
  `phase-27-leftovers-sweep` (final tree `66445ed`, code-complete 2026-09-22, unmerged). Two things the
