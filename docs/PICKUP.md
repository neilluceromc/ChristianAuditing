...
1. **Phase 23 — Parkinson sweep: quick forms and complete-by dates — IS CODE-COMPLETE on
   `phase-23-parkinson-sweep` at final tree `09bf0a5` (after the final-review fix wave and one
   test-only battery fix); UNMERGED and UNPUSHED; branch and worktree still in place** (10 tasks,
   `D-1`…`D-20` — the final-review fix wave added `D-16`…`D-19`, the final battery `D-20`) —
   **merging, pushing and the staging redeploy are the user's decisions, not pre-authorised.**
   ...
   intent. Local `main` also still carries two unpushed docs-only commits, `39f73a3` (spec) and
   ...
   plan `D-15`.
2. **Phase 22 — Stock control D2: FIFO lot consumption and costing, optional expiry, three report pages
...
3. **Phase 21 — Approvals oversight: direct IT changes made visible, plus five correctness and quality
   ...
   the battery.
4. **Phase 20 — IT side: transfers, offboarding attribution and facets, directory quality, six known
...
5. **Phase 19 — Stock control, part D1 (the ledger core) — is DONE, including the final-review fix
   ...
   **merged to `main` via `--no-ff` `d311e26` and pushed (2026-09-10); branch and worktree removed; staging
   ...
   is next (item 9 below); Purchasing's opening stock goes in through `/stock/import`.**
6. **Phase 18 — Purchasing: suppliers and request extensions — is DONE, including the
...
7. **Deploy the prototype to the staging laptop and let Purchasing and Finance use it.** Chosen by the user
   ...
   scan one QR).
8. **Two visual checks never done by a human:** the inventory class-switch chips and Finance's IT/Purchasing
   tabs by eye, and the Register form signed in as `purchasing@`. Both are asserted by e2e and axe, never
   looked at.
9. **Remaining candidates**, in value order (**Phase 16 delivered the fourth item this list used to
...
10. **The §9 subsystems from the Admin meeting — B and C shipped in Phase 18; D's first half (D1) shipped
   in Phase 19 (item 5 above); D2 shipped in Phase 22 (merged `26aaf82`, item 2 above).** Vendor master data (B)
   ...
   **D1 · stock control** (items, the ledger, derived balances, stocktake, import) is merged to `main`
   ...
   merged to `main` via `--no-ff` `26aaf82` and pushed 2026-09-16 (`HANDOVER.md` §9, item 2 above).
