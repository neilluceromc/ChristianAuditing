...
| Database | **20 migrations**, additive only, on `main` (migration 20 `stock_control`); staging remains at **19** until the next `-Force` redeploy. `prisma migrate reset` is **not** used in this project. |
...
1. **Phase 19 — Stock control, part D1 (the ledger core) — is DONE, including the final-review fix
