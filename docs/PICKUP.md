...
1. **Phase 30 — Laws of UX applied to the inventory area (the record's state-chosen primary with
   Return on held devices and Edit kept, one Register flow, a row menu and attention markers on
   the list, the role's own class by default) IS CODE-COMPLETE on `phase-30-inventory-uiux` at
   final tree `4e6fc7f` (after the final-review fix wave), UNMERGED and UNPUSHED.** Spec
   ...
   table (409 e2e / 39 files, 1624 unit / 95 files, 26 migrations). **The finishing decision is the
   user's:** merge `--no-ff` into `main`, then compare the branch tip's tree with the merge commit's
   tree (the root checkout has no `node_modules`, so the tree-hash check stands in for a re-test),
   then push **only `main`**, which also carries the two docs-only commits `a94f47a` and `b582535`.
   **The staging redeploy is a separate decision** — the phase adds **no migration** (26 stays), so it
   is a code-only `scripts/deploy-staging.ps1 -Force`. `inventory_dev` is clean: `npm run db:seed`
...
- **The Attention sort is an in-memory candidate pass (Phase 30).** `?sort=attention` on `/inventory` reads every asset the filters admit and orders them in memory, like Phase 29's Loadout sort — fine at today's size, to revisit past a few thousand assets (spec §7). Recorded with it: Export ignores the Attention sort (it orders in SQL); the Loadout sort (`src/server/modules/employees/queries.ts:71`) still finds its key anywhere in the sort, the pattern Phase 30's I-2 fixed for Attention; and `src/server/modules/import/asset-actions.ts`'s `revalidatePath("/inventory/[id]", "page")` patterns omit the route groups and should be verified. Full facts: `HANDOVER.md` (w).
