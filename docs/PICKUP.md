...
| Branch | `main`, **level with `origin/main`** after the Phase 19 push (merge `d311e26`, 2026-09-10) — count with `git rev-list --count origin/main..main` before trusting this; `main` is otherwise unchanged at `93291f0`. Phase branches (`phase-10-polish` … `phase-14-department-owned-classes`) exist only on the old dev laptop; `phase-15-direct-it-lifecycle`, `phase-16-registration-custody`, `phase-17-scale-sweep`, `phase-18-purchasing-suppliers` and `phase-19-stock-control` were merged and deleted on the staging laptop. All are fully contained in `main`. **`phase-20-it-people-and-gaps` exists locally, CODE-COMPLETE, UNMERGED and UNPUSHED** (9 tasks, `D-1`…`D-22`; battery in the table below) — merging and pushing are the user's decisions. **Staging still runs the Phase 18 merge** (`4cf5697`) since the 2026-09-09 forced redeploy (migration 19 applied; 16–18 came with the two 2026-09-08 redeploys); **Phase 19's migration 20 applies at the next `scripts/deploy-staging.ps1 -Force`**, and Phase 20's migration 21 will ride the same redeploy once merged — `-Force` is needed whenever the merge happened on this laptop, because the plain run sees nothing new and skips. |
...
| Database | **21 migrations** on `phase-20-it-people-and-gaps` (migration 21 `employee_transfers`, additive); `main` remains at **20** until Phase 20 is merged; staging remains at **19** until the next `-Force` redeploy. `prisma migrate reset` is **not** used in this project. |
| Battery, last run 2026-09-10 (Phase 20 branch tip `f6ec5e1`, `phase-20-it-people-and-gaps`, unmerged) | `tsc` clean · `lint` clean · **21 migrations** · **1291 unit / 75 files** · **296 e2e / 27 files** by `--list`. Task 9's close ran six foreground chunks, `E2E_PORT=3100 --workers=1 --global-timeout=540000`: **A 54 passed (3.3m) · B 67 passed (3.9m) · C 68 passed (4.4m) · D 38 passed (2.6m) · E 53 passed (7.3m) · F 21 passed (2.0m) = 301 run / 296 unique** (chunks C and F both run `offboarding-v2.spec.ts`'s 5 cases by the brief's own chunk lists), zero failed, zero did-not-run on the runs recorded. Chunks A, C and D each failed a first pass on a stale pre-Phase-20 e2e expectation, were reproduced once as-is, fixed in their own commit (`fe4295c`, `63df64a`, then `0b463b1` and `f6ec5e1` for chunk D's two), and re-ran clean; chunks B, E, F passed clean first try. Commands and history: `HANDOVER.md` §0 item 9; what shipped and each fix's diagnosis: `HANDOVER.md` (m). |
...
1. **Phase 20 — IT side: transfers, offboarding attribution and facets, directory quality, six known
   gaps — is CODE-COMPLETE on `phase-20-it-people-and-gaps`, UNMERGED and UNPUSHED** (9 tasks,
   `D-1`…`D-22`; branch tip `f6ec5e1`) — **merging, pushing and the staging redeploy are the user's
   decisions, not pre-authorised.** See §1's table and `HANDOVER.md` (m) for what shipped and the battery.
