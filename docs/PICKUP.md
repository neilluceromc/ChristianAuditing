...
1. **Phase 26 — holds that work + a repair end-date: reserve a spare from the asset record or from
   ...
   final whole-branch review added `D-8`, the fix wave `D-9`, the final battery `D-10`). **Staging is
   NOT yet redeployed — that redeploy is the user's decision, not pre-authorised.** When asked, run
   `scripts/deploy-staging.ps1 -Force` (`-Force` because the merge happened on this laptop) — and
   **this redeploy is not code-only: migration 25 `repair_end_and_holds` applies on it** (additive, one
   ...
   DEFECTIVE, so nothing is invented and `BR-MN-0911`-style rows keep their dash). The push also
...
- **Closed in Phase 26 — holds that work, and a repair end-date.** Phase 25's audit turned two
  ...
  `phase-26-holds-and-repair-end` delivered both (final tree `1442289`; merged to `main` as `a5883ad` and pushed 2026-09-22; staging not yet redeployed).
