...
| Database | **23 migrations** on `main` (migration 23 `stock_lots_and_allocations`, merged 2026-09-16: additive, with an asserted plpgsql backfill of D1's history — a lot for every lot-less inflow, allocations for every outflow); staging is still at **22** (migration 23 applies at the next `-Force` redeploy). `prisma migrate reset` is **not** used in this project. |
...
1. **Phase 22 — Stock control D2: FIFO lot consumption and costing, optional expiry, three report pages
   ...
   **Staging is NOT yet redeployed: migration 23 (with its asserted backfill) applies at the next
   `scripts/deploy-staging.ps1 -Force`, the user's decision, not pre-authorised.** See §1's table above and `HANDOVER.md` (o) for
...
9. **The §9 subsystems from the Admin meeting — B and C shipped in Phase 18; D's first half (D1) shipped
   in Phase 19 (item 4 above); D2 shipped in Phase 22 (merged `26aaf82`, item 1 above).** Vendor master data (B)
   ...
   **D1 · stock control** (items, the ledger, derived balances, stocktake, import) is merged to `main`
   ...
   merged to `main` via `--no-ff` `26aaf82` and pushed 2026-09-16 (`HANDOVER.md` §9, item 1 above).
