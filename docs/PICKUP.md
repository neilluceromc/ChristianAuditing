...
| Branch | `main`, **level with `origin/main`** after the Phase 16 push (merge `83643b4`, 2026-09-08) — count with `git rev-list --count origin/main..main` before trusting this. Phase branches (`phase-10-polish` … `phase-14-department-owned-classes`) exist only on the old dev laptop; `phase-15-direct-it-lifecycle` and `phase-16-registration-custody` were merged and deleted on the staging laptop. All are fully contained in `main`. **Staging runs `fada254`** since the 2026-09-08 forced redeploy (migrations 16 and 17 applied) — use `scripts/deploy-staging.ps1 -Force` whenever the merge happened on this laptop, because the plain run sees nothing new and skips. |
...
1. **Phase 17 — the scale sweep — is DONE.** Code-complete on `phase-17-scale-sweep`, 2026-09-08
   (UNMERGED and UNPUSHED at the time of writing): pagination on Approvals, Offboarding, Reservations,
   ...
   `superpowers/plans/2026-09-08-phase-17-scale-sweep.md` (9 amendments, `D-1`…`D-9`). What remains: the
   merge/push and the staging redeploy, both already authorised by the user for when this phase is done.
...
- **The merged-timeline cursor's `skip` URL parameter is unclamped.** `parseTimelineCursor` accepts whatever `skip` a hand-edited URL supplies, and `mergeTimeline` fetches `take = limit + 1 + skip` per source — so a large hand-crafted `skip` inflates one Prisma fetch. Flagged during Phase 17 (plan `D-8`'s fix); a follow-up should clamp it (e.g. to 10,000).
