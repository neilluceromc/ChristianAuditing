...
/**
 * Phase 20: one runner per area (mirrors use-stock-runner.ts / use-supplier-
 * runner.ts — same file, same note). Every mutation returns the house
 * ActionResult union: rate_limited -> countdown, validation -> field errors
 * (any key the caller doesn't claim with a FormField falls back into the
 * banner so it never dead-ends silently), anything else (conflict/forbidden)
 * -> banner.
 *
 * `refresh` defaults to true (`router.refresh()` after a successful
 * mutation).
 *
 * `okMsg` may be a plain string or a function of the result data.
 */
export function useEmployeeRunner(claimedFieldKeys: string[] = []) {
