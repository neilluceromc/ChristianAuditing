...
/*
 * Phase 30: the create half of what used to be `AssetForm` (mode="new"),
 * moved here verbatim when `asset-form.tsx` became edit-only. It serves
 * /inventory/new alone, and only until that route becomes a redirect to the
 * one Register flow (plan Task 5) — delete this file with that change.
 */
...
export function NewAssetForm({
  ...
  /** Per-category next-tag suggestion data (`tagSuggestions`). */
  ...
  const [form, setForm] = useState<NewAssetFields>({
  ...
  // Task 12: next-tag suggestion. `tagTouched` freezes the suggestion the
  // moment the tag field is hand-edited — a later category change must not
  // clobber a number the user already chose.
  const [tagTouched, setTagTouched] = useState(false);
  const [tagHint, setTagHint] = useState<string | null>(null);

  // Task 12: documents attached at registration — uploaded one by one,
  // after createAsset, once the asset id exists to attach them to.
  ...
  const set = (key: keyof NewAssetFields) => (value: string) => {
    ...
    <form onSubmit={submit} className="flex max-w-[720px] flex-col gap-4">
      ...
      <Card>
        ...
        <CardBody className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          {field("Asset tag", "tag", {
            required: true,
            hint: tagHint ?? "Format BR-XX-0000, as printed on the label.",
            placeholder: CLASS_EXAMPLE[cls].tag,
            onBlur: () => scheduleIdentifierCheck("tag", form.tag, tagCheckTimer),
            autoFocus: true,
          })}
          {field("Model", "model", { required: true, placeholder: CLASS_EXAMPLE[cls].model })}
          {field("Brand", "brand")}
          {field("Serial", "serial", {
            onBlur: () => scheduleIdentifierCheck("serial", form.serial, serialCheckTimer),
          })}
              ...
              <Select
                ...
                onChange={(e) => {
                  ...
                  setForm((f) => ({
                    ...
                    categoryId,
                    typeId: "",
                    tag: !tagTouched && run?.ok ? run.tags[0] : f.tag,
                  }));
                  setTagHint(
                    !categoryId
                      ? null
      ...
      <div className="flex items-center gap-3">
        <Button type="submit" variant="primary" loading={pending}>Register asset</Button>
      </div>
    </form>
  );
}
